...
class ActionMask:
    ...
    def __init__(self, config, logger: Optional[logging.Logger] = None):
        ...
        self.logger.info(f"Action masking initialized - max_position_ratio: {self.max_position_ratio}")
    
    def get_action_mask(self, portfolio_state: Dict[str, Any], market_state: Dict[str, Any]) -> np.ndarray:
        """
        ...
        for i, size_pct in enumerate(self.position_sizes):
            ...
            # Check constraints
            can_buy = (
                buy_value >= self.min_order_value and           # Minimum order size
                new_total_position <= max_position_value and    # Position limit
                cash >= buy_value                               # Sufficient cash
            )
            mask[4 + i] = can_buy
        ...
        if current_shares > 0 and sell_price > 0:
            for i, size_pct in enumerate(self.position_sizes):
                ...
                # Check constraints
                can_sell = (
                    shares_to_sell >= self.min_shares_to_sell and  # At least 1 share
                    sell_value >= self.min_order_value             # Minimum order value
                )
                mask[8 + i] = can_sell
        else:
    ...
    def mask_action_probabilities(self, action_probs: np.ndarray, portfolio_state: Dict[str, Any], 
    ...
    def is_action_valid(self, action_idx: int, portfolio_state: Dict[str, Any], 
    ...
    def get_valid_actions(self, portfolio_state: Dict[str, Any], 
                         market_state: Dict[str, Any]) -> list:
        """

...
class DashboardCallback(BaseCallback):
    ...
    def on_episode_step(self, step_data: Dict[str, Any]) -> None:
        ...
        if current_price > 0:
            ...
            self.dashboard_state.spread = (
                self.dashboard_state.ask_price - self.dashboard_state.bid_price
            )
            self.dashboard_state.spread_pct = (
                self.dashboard_state.spread
                / max(0.01, self.dashboard_state.current_price)
            ) * 100
        ...
        session_pnl = (
            ...
            or info.get("portfolio_realized_pnl_session_net", 0.0)
            or info.get("total_pnl", 0.0)
            or 0.0
        )
        ...
        self.dashboard_state.cumulative_reward = (
            getattr(self.dashboard_state, "cumulative_reward", 0.0)
            + self.dashboard_state.last_step_reward
        )
            ...
            try:
                ...
                if hasattr(timestamp, "tz_localize"):
                    if timestamp.tz is None:
                        ...
                        timestamp_ny = timestamp.tz_convert(
                            "America/New_York"
                        ).tz_localize(None)
                else:
    ...
    def on_episode_end(self, episode_num: int, episode_data: Dict[str, Any]) -> None:
        ...
        if episode_reward > mean_reward * 1.5:
            self._add_training_event(
                f"High reward episode: {episode_reward:.2f}", "success"
            )
        elif episode_data.get("termination_reason") == "bankruptcy":
            self._add_training_event(
                f"Episode {episode_num} ended in bankruptcy", "danger"
            )
    ...
    def on_update_end(self, update_num: int, update_metrics: Dict[str, Any]) -> None:
        ...
        self.dashboard_state.policy_loss_history.append(
            self.dashboard_state.policy_loss
        )
        ...
        self.dashboard_state.kl_divergence_history.append(
            self.dashboard_state.kl_divergence
        )
        self.dashboard_state.clip_fraction_history.append(
            self.dashboard_state.clip_fraction
        )
        self.dashboard_state.learning_rate_history.append(
            self.dashboard_state.learning_rate
        )
        self.dashboard_state.explained_variance_history.append(
            self.dashboard_state.explained_variance
        )
        self.dashboard_state.mean_episode_reward_history.append(
    ...
    def on_evaluation_end(self, eval_results: Dict[str, Any]) -> None:
        ...
        self._add_training_event(
            f"Evaluation: reward={eval_results.get('mean_reward', 0):.2f}", "primary"
        )
    ...
    def on_order_filled(self, fill_data: Dict[str, Any]) -> None:
        ...
        if hasattr(self.dashboard_state, "recent_slippage"):
            ...
            self.dashboard_state.avg_slippage = np.mean(
                self.dashboard_state.recent_slippage
            )
    ...
    def on_position_closed(self, trade_result: Dict[str, Any]) -> None:
        ...
        # Update win rate
        self.dashboard_state.win_rate = (
            self.dashboard_state.session_winning_trades
    ...
    def on_model_forward(self, forward_data: Dict[str, Any]) -> None:
        ...
        if "action_probs" in forward_data and hasattr(
            self.dashboard_state, "action_probabilities"
        ):
            self.dashboard_state.action_probabilities = forward_data[
                "action_probs"
            ].tolist()

        # Update feature stats periodically (if attribute exists)
        if (
            self.total_episodes % 100 == 0
            and "feature_stats" in forward_data
            and hasattr(self.dashboard_state, "feature_stats")
        ):
    ...
    def on_momentum_day_change(self, event_data: Dict[str, Any]) -> None:
        ...
        self.dashboard_state.current_momentum_day_quality = day_info.get(
            "quality_score", 0.0
        )
    ...
    def on_custom_event(self, event_name: str, event_data: Dict[str, Any]) -> None:
        ...
        if event_name == "training_update":
            ...
            self.dashboard_state.overall_progress = event_data.get(
            ...
            self.dashboard_state.value_loss_history.append(
                self.dashboard_state.value_loss
            )
            self.dashboard_state.entropy_history.append(self.dashboard_state.entropy)
            ...
            self.dashboard_state.cycles_completed = event_data.get(
                "cycles_completed", 0
            )
            self.dashboard_state.target_cycles_per_day = event_data.get(
                "target_cycles_per_day", 10
            )
            self.dashboard_state.cycles_remaining_for_day_switch = event_data.get(
                "cycles_remaining_for_day_switch", 10
            )
            self.dashboard_state.episodes_on_current_day = event_data.get(
                "episodes_on_current_day", 0
            )
            self.dashboard_state.day_switch_progress_pct = event_data.get(
    ...
    def on_reset_point_selected(self, tracking_data: Dict[str, Any]) -> None:
        ...
        self.dashboard_state.selected_reset_point_index = tracking_data.get(
            "selected_index", 0
        )
        self.dashboard_state.selected_reset_point_timestamp = tracking_data.get(
            "selected_timestamp", ""
        )
        self.dashboard_state.total_available_points = tracking_data.get(
            "total_available_points", 0
        )
        self.dashboard_state.points_used_in_cycle = tracking_data.get(
            "points_used_in_cycle", 0
        )
        self.dashboard_state.points_remaining_in_cycle = tracking_data.get(
            "points_remaining_in_cycle", 0
        )
        self.dashboard_state.current_roc_score = tracking_data.get("roc_score", 0.0)
        self.dashboard_state.current_activity_score = tracking_data.get(
            "activity_score", 0.0
        )
    ...
    def on_training_end(self, final_stats: Dict[str, Any]) -> None:
        ...
        if hasattr(self.dashboard_state, "final_mean_reward"):
            self.dashboard_state.final_mean_reward = (

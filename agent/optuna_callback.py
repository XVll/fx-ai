...
class OptunaCallback(BaseCallback):
    ...
    def on_training_start(self, config: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"Optuna trial {self.trial.number} started - optimizing {self.metric_name}")
    ...
    def on_training_end(self, final_stats: Dict[str, Any]) -> None:
        ...
        if self.metric_name == "mean_reward":
            if self.eval_rewards:
                ...
                final_metric = np.mean(self.episode_rewards[-50:]) if self.episode_rewards else float('-inf')
        else:

...
class WandBCallback(BaseCallback):
    ...
    def on_position_closed(self, trade_result: Dict[str, Any]) -> None:
        ...
        # Calculate win rate
        win_rate = (self.portfolio_stats['winning_trades'] / 
        ...
        if len(self.trade_history) % 10 == 0:
            wandb.log({'trades_table': wandb.Table(
                columns=list(self.trade_history[0].keys()),
                data=[list(t.values()) for t in self.trade_history[-50:]]  # Last 50 trades
            )})
    ...
    def on_portfolio_update(self, portfolio_state: Dict[str, Any]) -> None:
        ...
        drawdown = (self.portfolio_stats['peak_equity'] - equity) / self.portfolio_stats['peak_equity']
        self.portfolio_stats['max_drawdown'] = max(self.portfolio_stats['max_drawdown'], drawdown)
    ...
    def on_model_forward(self, forward_data: Dict[str, Any]) -> None:
        ...
        if 'action_probs' in forward_data:
            ...
            metrics = {f'model/action_prob_{i}': prob 
    ...
    def on_attribution_computed(self, attribution_data: Dict[str, Any]) -> None:
        ...
        if 'feature_importances' in attribution_data:
            ...
            top_features = sorted(importances.items(), key=lambda x: abs(x[1]), reverse=True)[:20]
        ...
        if 'dead_features' in attribution_data:
            wandb.log({
                'attribution/dead_feature_count': len(attribution_data['dead_features']),
                'attribution/dead_feature_ratio': attribution_data.get('dead_feature_ratio', 0.0),
                'update': self.total_updates
            })
    ...
    def on_curriculum_stage_change(self, stage_info: Dict[str, Any]) -> None:
        ...
        wandb.log({
            ...
            'curriculum/performance_threshold': stage_info.get('performance_threshold', 0.0),
            'episode': self.total_episodes
        })

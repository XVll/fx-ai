...
class ConfigLoader:
        ...
        if override_file:
            ...
            if override_path.exists():
                ...
                raise FileNotFoundError(
                    f"Config override file not found: {override_file}"
                )

        # Validate final config
        self.logger.debug(f"Config type before validation: {type(config)}")
        self.logger.debug(
            f"Config.model type: {type(config.model) if hasattr(config, 'model') else 'No model attr'}"
        )
    ...
    def _validate_config(self, config: Config):
        ...
        try:
            ...
            self.logger.info(
                f"Action space: {action_types} types × {position_sizes} sizes = {expected_actions} total actions"
            )
        except Exception as e:
        ...
        if config.env.reward.enable_pnl_reward:
            enabled_components.append(
                f"pnl (coeff: {config.env.reward.pnl_coefficient})"
            )
        if config.env.reward.enable_holding_penalty:
            enabled_components.append(
                f"holding_penalty (coeff: {config.env.reward.holding_penalty_coefficient})"
            )
        if config.env.reward.enable_drawdown_penalty:
            enabled_components.append(
                f"drawdown_penalty (coeff: {config.env.reward.drawdown_penalty_coefficient})"
            )
        if config.env.reward.enable_profit_giveback_penalty:
            enabled_components.append(
                f"profit_giveback_penalty (coeff: {config.env.reward.profit_giveback_penalty_coefficient})"
            )
        if config.env.reward.enable_max_drawdown_penalty:
            enabled_components.append(
                f"max_drawdown_penalty (coeff: {config.env.reward.max_drawdown_penalty_coefficient})"
            )
        if config.env.reward.enable_profit_closing_bonus:
            enabled_components.append(
                f"profit_closing_bonus (coeff: {config.env.reward.profit_closing_bonus_coefficient})"
            )
        if config.env.reward.enable_clean_trade_bonus:
            enabled_components.append(
                f"clean_trade_bonus (base_multiplier: {config.env.reward.base_multiplier})"
            )
        enabled_components.append(
            f"bankruptcy_penalty (coeff: {config.env.reward.bankruptcy_penalty_coefficient})"
        )

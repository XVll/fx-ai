...
class EMAInteractionPatternFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            alignment_consistency = np.sum(
                ema9_above_ema20 == ema9_above_ema20[0]
            ) / len(ema9_above_ema20)
    ...
    def _count_ema_touches(
        self, prices: np.ndarray, ema: np.ndarray, threshold: float = 0.003
    ) -> int:
...
class EMACrossoverDynamicsFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            cross_speed = (
                abs(ema9[last_cross_idx] - ema20[last_cross_idx])
                / ema20[last_cross_idx]
            )
            ...
            if bars_after_cross > 1:
                if is_bullish_cross:
                    follow_through = np.mean(
                        ema9[last_cross_idx:] > ema20[last_cross_idx:]
                    )
                else:
                    follow_through = np.mean(
                        ema9[last_cross_idx:] < ema20[last_cross_idx:]
            ...
            pre_cross_volume = np.mean(
                volumes[max(0, last_cross_idx - 3) : last_cross_idx]
            )
            ...
            pre_cross_momentum = (
                closes[last_cross_idx] - closes[max(0, last_cross_idx - 3)]
            ) / closes[max(0, last_cross_idx - 3)]
            post_cross_momentum = (closes[-1] - closes[last_cross_idx]) / closes[
                last_cross_idx
            ]

            momentum_consistency = (
                1.0 if pre_cross_momentum * post_cross_momentum > 0 else 0.0
            )
...
class EMATrendAlignmentFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            ema9_1m_slope = (
                (ema9_1m[-1] - ema9_1m[-5]) / ema9_1m[-5] if len(ema9_1m) >= 5 else 0
            )
            ema20_1m_slope = (
                (ema20_1m[-1] - ema20_1m[-5]) / ema20_1m[-5]
                if len(ema20_1m) >= 5
                else 0
            )
            ema9_5m_slope = (
                (ema9_5m[-1] - ema9_5m[-3]) / ema9_5m[-3] if len(ema9_5m) >= 3 else 0
            )
            ema20_5m_slope = (
                (ema20_5m[-1] - ema20_5m[-3]) / ema20_5m[-3]
                if len(ema20_5m) >= 3
                else 0
            )
            ...
            alignment_strength = (
                ...
                + slope_alignment * 0.3  # Slope alignment
                + min((tf1_strength + tf5_strength) * 25, 1.0)
                * 0.3  # EMA separation strength
            )

...
if __name__ == '__main__':
    ...
    logger = logging.getLogger("FeatureExtractorTest")


    # Dummy market state (similar to what MarketSimulatorV2 would provide)
    def generate_dummy_1s_events(start_dt: datetime, num_seconds: int) -> List[Dict]:
        events = []
        for i in range(num_seconds):
            ts = start_dt + timedelta(seconds=i)
            o, h, l, c, v = np.random.rand(5) * np.array([100, 2, -2, 100, 1000]) + np.array([50, 100, 100, 50, 100])
            bar = {'open': o, 'high': h, 'low': l, 'close': c, 'volume': int(v)}
            trades = [{'price': c, 'size': np.random.randint(1, 5)} for _ in range(np.random.randint(0, 3))]
            events.append({'timestamp': ts, 'bar': bar, 'trades': trades, 'quotes': []})
        return events


    total_simulation_seconds = 1000  # Approx 16 minutes
    start_datetime = datetime(2023, 1, 1, 9, 30, 0)

    all_dummy_events = generate_dummy_1s_events(start_datetime, total_simulation_seconds)

    fe_config = {
        'hf_steps': 60, 'hf_feature_size': 8,
        'mf_periods': 5, 'mf_period_seconds': 60, 'mf_feature_size': 10,  # Shortened for quick test
        'lf_periods': 3, 'lf_period_seconds': 300, 'lf_feature_size': 10,  # Shortened for quick test
        'static_feature_size': 5,
    }
    feature_extractor = FeatureExtractorV2(config=fe_config, logger=logger)

    # Simulate stepping through time
    # The rolling_1s_data_window in market_state would grow and then roll
    # Max window size for MarketSimulatorV2 should be enough for the longest lookback
    # e.g., lf_periods * lf_period_seconds + some buffer

    sim_window_size = fe_config['lf_periods'] * fe_config['lf_period_seconds'] + 60  # Example

    for i in range(60 * 4, total_simulation_seconds):  # Start after some data has accumulated
        current_time_step = start_datetime + timedelta(seconds=i)

        # Simulate the rolling window from MarketSimulatorV2
        start_idx_for_window = max(0, i - sim_window_size + 1)
        current_rolling_1s_window = all_dummy_events[start_idx_for_window: i + 1]

        market_state_now = {
            'timestamp': current_time_step,
            'rolling_1s_data_window': current_rolling_1s_window,
            # 'historical_5m_for_sr': None, # Add if needed for static features
            # 'historical_1d_for_sr': None,
        }

        # Dummy portfolio state
        portfolio_state_now = {'position': (i % 3) - 1, 'total_pnl': i * 0.1}

        if i % 60 == 0:  # Log less frequently
            logger.info(f"\n--- Extracting features for timestamp: {current_time_step} ---")
            logger.info(f"Rolling 1s window size: {len(current_rolling_1s_window)}")

        features = feature_extractor.extract_features(market_state_now, portfolio_state_now)

        if i % 60 == 0:
            logger.debug(f"HF Features shape: {features['hf_features'].shape}")
            logger.debug(f"MF Features shape: {features['mf_features'].shape}")
            # logger.debug(f"MF Features (first period):\n{features['mf_features'][0]}")
            # logger.debug(f"MF Features (last period):\n{features['mf_features'][-1]}")
            logger.debug(f"LF Features shape: {features['lf_features'].shape}")
            # logger.debug(f"LF Features (first period):\n{features['lf_features'][0]}")
            # logger.debug(f"Static Features: {features['static_features']}")
            logger.info(f"Completed MF Aggregates in FE: {len(feature_extractor.completed_mf_period_features)}")
            logger.info(f"Completed LF Aggregates in FE: {len(feature_extractor.completed_lf_period_features)}")
            if feature_extractor.completed_lf_period_features:
                logger.debug(
                    f"Last completed LF period features in deque: {feature_extractor.completed_lf_period_features[-1]}")

    logger.info("Test simulation finished.")
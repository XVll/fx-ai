...
class PatternFeatures:
    ...
    def calculate(self, market_data: Dict[str, Any], context: str = 'mf') -> Dict[str, float]:
        ...
        features['swing_high_price_pct'] = (swing_high_price - current_price) / current_price if current_price > 0 else 0
        features['swing_low_price_pct'] = (current_price - swing_low_price) / current_price if current_price > 0 else 0
        features['bars_since_swing_high'] = len(highs) - 1 - swing_high_idx if swing_high_idx >= 0 else len(highs)
        features['bars_since_swing_low'] = len(lows) - 1 - swing_low_idx if swing_low_idx >= 0 else len(lows)
        ...
        features['consolidation_score'] = np.std(closes[-10:]) / np.mean(closes[-10:]) if len(closes) >= 10 and np.mean(closes[-10:]) > 0 else 0
        ...
        features['breakout_potential'] = vol_surge * (1 / (features['range_compression'] + 0.1))

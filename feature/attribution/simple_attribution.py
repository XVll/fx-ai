...
class SimpleFeatureAttribution:
        ...
        try:
            ...
            results = {
                ...
                "summary": self._create_summary(
                    input_gradients, permutation_importance, variance_importance
                ),
            }
            ...
            self.logger.info(
                f"✅ Simple attribution analysis completed for {len(analysis_states)} states"
            )
            return results
    ...
    def _calculate_input_gradients(
        ...
        for state in states:
            ...
            for branch, tensor in state.items():
                if torch.is_tensor(tensor):
                    state_copy[branch] = (
                        tensor.clone().detach().requires_grad_(True).to(self.device)
                    )
                else:
    ...
    def _calculate_variance_importance(
        self, states: List[Dict[str, torch.Tensor]]
    ) -> Dict[str, float]:
    ...
    def _get_top_features(
        self, gradients: Dict[str, float], top_k: int = 3
    ...
    def _create_summary(
        self, gradients: Dict, permutation: Dict, variance: Dict
    ) -> Dict[str, Any]:
        ...
        all_branches = (
            set(gradients.keys()) | set(permutation.keys()) | set(variance.keys())
        )
        overall_scores = {}
    ...
    def get_attribution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_branch": max(recent_importance.items(), key=lambda x: x[1]["current"])[
                0
            ]

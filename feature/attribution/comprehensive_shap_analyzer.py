...
try:
    ...
    from captum.attr import (
        ...
        FeatureAblation, Occlusion, LayerConductance,
...
class AttributionConfig:
    ...
    # Methods to use
    methods: List[str] = field(default_factory=lambda: ["gradient_shap", "integrated_gradients"])
...
class ComprehensiveSHAPAnalyzer:
    ...
    def __init__(
        ...
        if not ATTRIBUTION_AVAILABLE:
            raise ImportError("SHAP and Captum are required. Install with: pip install shap captum")
            
        self.model = model
        self.feature_names = feature_names
        self.branch_configs = branch_configs
        self.config = config or AttributionConfig()
        self.device = device or torch.device("cuda" if torch.cuda.is_available() and self.config.use_gpu else "cpu")
        ...
        for branch, (seq_len, feat_dim) in self.branch_configs.items():
            ...
            for seq_pos in range(seq_len):
                for feat_pos in range(feat_dim):
                    ...
                    self.feature_index_map[f"{branch}.{feat_name}.{seq_pos}"] = current_idx
    ...
    def _setup_explainers(self):
        ...
        if "integrated_gradients" in self.config.methods:
            self.explainers["integrated_gradients"] = IntegratedGradients(self.model_wrapper)
            ...
            if end_idx > tensor.shape[1]:
                ...
                if available > 0:
                    ...
                    if available < expected_size:
                        padding = torch.zeros(
                            batch_size, expected_size - available, 
    ...
    def _setup_tracking(self):
        ...
        # Attribution history
        self.attribution_history = defaultdict(lambda: deque(maxlen=self.config.history_length))
        self.method_history = defaultdict(lambda: deque(maxlen=self.config.history_length))
        
        # Feature importance tracking
        self.global_importance = np.zeros(self.total_features)
        self.importance_ema = np.zeros(self.total_features)  # Exponential moving average
            ...
            try:
                if method_name in ["gradient_shap", "noise_tunnel"]:
                    ...
                    attributions = explainer.attribute(
                        test_input,
                        baselines=self.background_cache[:2],
                        target=0
                    )
                else:
                ...
                self.logger.debug(f"✓ {method_name} working: shape {attributions.shape}")
    ...
    def analyze_features(
        ...
        if not self.background_cache:
            self.logger.error("No background data available. Call setup_background() first.")
        ...
        try:
            ...
            primary_attributions = all_attributions.get(
                ...
                list(all_attributions.values())[0]
            )
            ...
            results = {
                ...
                # Feature importance
                "feature_importance": self._calculate_feature_importance(primary_attributions),
                "branch_importance": self._calculate_branch_importance(primary_attributions),
                ...
                ) if self.config.analyze_interactions else None,
                ...
                # Method comparison
                "method_comparison": self._compare_attribution_methods(all_attributions),
                
                # Outlier detection
                "outliers": self._detect_outliers(primary_attributions) if self.config.detect_outliers else None
            }
            ...
            if self.config.save_plots and self.analysis_count % self.config.dashboard_update_freq == 0:
    ...
    def _calculate_feature_importance(self, attributions: np.ndarray) -> Dict[str, float]:
        ...
        alpha = 0.1
        self.importance_ema = alpha * global_importance + (1 - alpha) * self.importance_ema
        
        # Update variance tracking
        self.importance_variance = alpha * (global_importance - self.importance_ema)**2 + \
        ...
        for key in importance_dict:
            ...
            importance_dict[key]["stability"] = 1.0 / (1.0 + importance_dict[key]["variance"])
            
        return importance_dict
        
    def _calculate_branch_importance(self, attributions: np.ndarray) -> Dict[str, Dict[str, float]]:
    ...
    def _get_top_features(self, attributions: np.ndarray) -> List[Dict[str, Any]]:
        ...
        for rank, idx in enumerate(top_indices):
            ...
            feature_info = {
                ...
                "positive_attributions": float(np.mean(feature_attrs[feature_attrs > 0]) if np.any(feature_attrs > 0) else 0),
                "negative_attributions": float(np.mean(feature_attrs[feature_attrs < 0]) if np.any(feature_attrs < 0) else 0),
                "attribution_range": [float(np.min(feature_attrs)), float(np.max(feature_attrs))]
            }
        ...
        try:
            ...
            return {
                ...
                "interaction_summary": {
                    ...
                    "max_correlation": float(np.max(np.abs(attr_corr[np.triu_indices_from(attr_corr, k=1)]))),
                    "features_with_interactions": len([m for m in self.feature_metadata.values() if m.interaction_partners])
                }
            }
    ...
    def _detect_dead_features(self, attributions: np.ndarray) -> Dict[str, Any]:
        ...
        for idx in list(self.dead_feature_candidates):
            ...
            if len(meta.importance_history) >= 5:
                ...
                if all(imp < self.config.dead_feature_threshold for imp in recent_importance):
        ...
        return {
            ...
            "dead_percentage": len(self.confirmed_dead_features) / self.total_features * 100,
            ...
            "summary": {
                branch: {
                    ...
                    "dead_percentage": len(features) / len(self.branch_feature_indices[branch]) * 100
                }
        ...
        for i in range(min(5, len(attributions))):  # Limit to 5 samples
            ...
            explanation = {
                ...
                "top_positive_features": [
                    ...
                    for idx in top_positive_idx if sample_attrs[idx] > 0
                ],
                ...
                "top_negative_features": [
                    ...
                    for idx in top_negative_idx if sample_attrs[idx] < 0
                ],
                
                # Summary statistics
                "total_positive_attribution": float(np.sum(sample_attrs[sample_attrs > 0])),
                "total_negative_attribution": float(np.sum(sample_attrs[sample_attrs < 0])),
    ...
    def _calculate_statistics(self, attributions: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "importance_stats": {
                ...
                "gini_coefficient": float(self._calculate_gini(np.mean(np.abs(attributions), axis=0))),
                "concentration_ratio": float(self._calculate_concentration_ratio(np.mean(np.abs(attributions), axis=0)))
            },
            
            "sparsity_stats": {
                "zero_attribution_ratio": float(np.mean(np.abs(attributions) < 1e-6)),
                "low_attribution_ratio": float(np.mean(np.abs(attributions) < self.config.dead_feature_threshold)),
    ...
    def _compare_attribution_methods(self, all_attributions: Dict[str, np.ndarray]) -> Dict[str, Any]:
        ...
        for i, method1 in enumerate(methods):
            for method2 in methods[i+1:]:
                overlap = len(
                    top_features_by_method[method1] & 
                    top_features_by_method[method2]
                )
    ...
    def _detect_outliers(self, attributions: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_outliers": sorted(outliers, key=lambda x: x["z_score"], reverse=True)[:10]
        }
    ...
    def _generate_visualizations(self, results: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            if results.get("feature_interactions") and "interaction_matrix" in results["feature_interactions"]:
            ...
            # 6. Importance trends
            if "importance_trends" in results and results["importance_trends"]["available"]:
    ...
    def _plot_importance_heatmap(self, feature_importance: Dict, timestamp: int) -> str:
        ...
        # Create figure
        fig, axes = plt.subplots(len(branch_data), 1, figsize=(12, 4 * len(branch_data)))
    ...
    def get_summary_for_logging(self) -> Dict[str, Any]:
        ...
        top_k_indices = np.argsort(current_importance)[-self.config.top_k_features:][::-1]
        ...
        summary = {
            ...
            # Performance metrics
            "avg_analysis_time": float(np.mean(self.analysis_times)) if self.analysis_times else 0,
            "avg_samples_analyzed": float(np.mean(self.sample_counts)) if self.sample_counts else 0,
            ...
            "dead_features": {
                ...
                "percentage": len(self.confirmed_dead_features) / self.total_features * 100,
                "by_branch": {
                    branch: len([
                        idx for idx in self.confirmed_dead_features 
                        if idx in indices
                    ])
                    for branch, indices in self.branch_feature_indices.items()
                }
    ...
    def log_to_wandb(self, results: Dict[str, Any], step: Optional[int] = None):
        ...
        try:
            ...
            # Dead features
            if "dead_features" in results:
                log_dict["shap/dead_features_count"] = results["dead_features"]["total_dead"]
                log_dict["shap/dead_features_percentage"] = results["dead_features"]["dead_percentage"]
                
                for branch, summary in results["dead_features"]["summary"].items():
                    log_dict[f"shap/dead_features_{branch}_count"] = summary["dead_count"]
            ...
            # Feature interactions
            if "feature_interactions" in results and "interaction_summary" in results["feature_interactions"]:
                summary = results["feature_interactions"]["interaction_summary"]
                log_dict["shap/interactions/mean_correlation"] = summary["mean_correlation"]
                log_dict["shap/interactions/max_correlation"] = summary["max_correlation"]
                log_dict["shap/interactions/features_with_interactions"] = summary["features_with_interactions"]
            ...
            # Method comparison
            if "method_comparison" in results and results["method_comparison"].get("comparison_available"):
                for comparison, metrics in results["method_comparison"]["method_agreements"].items():
                    log_dict[f"shap/method_comparison/{comparison}_agreement"] = metrics["agreement"]
                    log_dict[f"shap/method_comparison/{comparison}_correlation"] = metrics["correlation"]
    ...
    def save_state(self, path: str):
        ...
        state = {
            ...
            "attribution_history": dict(self.attribution_history)[-10:] if self.attribution_history else {}
        }

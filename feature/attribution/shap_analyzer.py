...
class ShapFeatureAnalyzer:
    ...
    def __init__(
        ...
        self.feature_stats = {
            ...
            "branch_sizes": {
                branch: len(names) for branch, names in feature_names.items()
            },
        }

        self.logger.info(
            f"🎯 SHAP Feature Analyzer initialized with {self.feature_stats['total_features']} features across {len(self.feature_stats['branches'])} branches"
        )
        ...
        try:
            for branch, (seq_len, feat_dim) in self.branch_configs.items():
                ...
                if end_idx > tensor.shape[1]:
                    self.logger.error(
                        f"Tensor reshape error: branch {branch} needs {expected_size} features "
                    ...
                    if available_size > 0:
                        ...
                        actual_features = (
                            available_size // seq_len if seq_len > 0 else feat_dim
                        )
                        branch_tensor = tensor[
                            :, start_idx : start_idx + (seq_len * actual_features)
                        ].reshape(batch_size, seq_len, actual_features)
            ...
            self.logger.error(
                f"Tensor shape: {tensor.shape}, Branch configs: {self.branch_configs}"
            )
            # Return simplified state dict
            state_dict = {
                "hf": tensor[:, :1200].reshape(batch_size, 60, 20)
                if tensor.shape[1] >= 1200
                else tensor.reshape(batch_size, 1, -1)
            }
    ...
    def setup_explainer(
        self, background_states: List[Dict[str, torch.Tensor]], method: str = "auto"
    ):
        """
        ...
        try:
            ...
            for state in background_states[
                :5
            ]:  # Use only 5 background samples for speed
                flat_data = self._state_dict_to_tensor(state)
                background_data.append(flat_data)

            background_array = np.array(background_data)
            self.logger.info(
                f"📊 Setting up SHAP explainer with background shape: {background_array.shape}"
            )
            self.logger.warning(
                f"⚠️ Using reduced background samples for performance. Full analysis would take too long with {background_array.shape[1]} features."
            )
            ...
            if method == "auto":
                ...
                if background_array.shape[1] > 1000:  # Many features - prioritize speed
                    methods_to_try = [
                        "partition",
                        "kernel",
                    ]  # Skip deep (requires tensorflow), try fastest first
                else:
                ...
                try:
                    if shap_method == "deep":
                        ...
                        explainer = shap.DeepExplainer(
                            self.model_wrapper, background_array
                        )
                        ...
                        explainer = shap.KernelExplainer(
                            self.model_wrapper, background_array[:3]
                        )  # Only 3 background samples
                        self.logger.info(
                            "✅ Using KernelSHAP explainer (performance-optimized)"
                        )
                        self.logger.warning(
                            f"⚠️ With {background_array.shape[1]} features, SHAP analysis will be approximate for speed"
                        )
    ...
    def analyze_features(
        self, states: List[Dict[str, torch.Tensor]], max_samples: int = 20
    ) -> Dict[str, Any]:
        """
        ...
        if not self.current_explainer:
            self.logger.error(
                "❌ No SHAP explainer available. Call setup_explainer() first."
            )
        ...
        try:
            ...
            self.logger.info(
                f"🔍 Analyzing {len(analysis_states)} states with SHAP (performance mode)..."
            )
            self.logger.warning(
                f"⚠️ Using only {len(analysis_states)} samples due to {analysis_array.shape[1]} features - full analysis would take too long"
            )
            ...
            try:
                ...
                if analysis_time > 60:  # More than 1 minute
                    self.logger.warning(
                        f"⚠️ SHAP analysis took {analysis_time:.1f}s - consider reducing frequency or features"
                    )

            except Exception as e:
                analysis_time = time.time() - start_time
                self.logger.error(
                    f"SHAP analysis failed after {analysis_time:.1f}s: {e}"
                )
            ...
            if isinstance(shap_values, list):
                ...
                shap_values_processed = np.array(shap_values).transpose(
                    1, 0, 2
            ...
            results = {
                ...
                "individual_explanations": self._get_individual_explanations(
                    primary_shap, analysis_array
                ),
            ...
            if self.save_plots:
                results["plot_paths"] = self._generate_visualizations(
                    primary_shap, analysis_array
                )
        ...
        for branch, feature_names in self.feature_names.items():
            ...
            for i, feature_name in enumerate(feature_names):
                if i < len(feature_importance):
                    importance[f"{branch}.{feature_name}"] = float(
    ...
    def _get_individual_explanations(
        self, shap_values: np.ndarray, analysis_data: np.ndarray
    ) -> List[Dict]:
        ...
        for i in range(min(5, len(shap_values))):
            ...
            explanation = {
                "sample_id": i,
                "top_positive": [
                    (int(idx), float(sample_shap[idx]), float(sample_data[idx]))
    ...
    def _detect_dead_features(
        self, shap_values: np.ndarray, threshold: float = 0.001
    ) -> Dict[str, List[str]]:
    ...
    def _map_index_to_feature(self, idx: int) -> Dict[str, str]:
        ...
        for branch, feature_names in self.feature_names.items():
            ...
            if idx < current_idx + branch_size:
                ...
                feature_name = (
                    feature_names[feat_pos]
                    if feat_pos < len(feature_names)
                    else f"feature_{feat_pos}"
                )
    ...
    def _prepare_visualization_data(
        ...
        return {
            ...
            "branch_ranking": sorted(
                branch_importance.items(), key=lambda x: x[1], reverse=True
            ),
        }
    ...
    def _generate_visualizations(
        ...
        try:
            ...
            if len(shap_values) > 0:
                ...
                shap.waterfall_plot(
                    shap.Explanation(
                        values=shap_values[0], base_values=0, data=analysis_data[0]
                    ),
                    show=False,
                    max_display=15,
                )
    ...
    def _update_attribution_history(self, results: Dict[str, Any]):
        ...
        if "summary_stats" in results:
            self.global_importance_history.append(
                results["summary_stats"]["mean_absolute_shap"]
            )
    ...
    def get_attribution_summary(self) -> Dict[str, Any]:
        ...
        # Top features
        top_features = sorted(
            recent_importance.items(), key=lambda x: x[1]["current"], reverse=True
        )[:10]
    ...
    def log_to_wandb(self, results: Dict[str, Any], step: int = None):
        ...
        try:
            ...
            if "dead_features" in results:
                total_dead = sum(
                    len(features) for features in results["dead_features"].values()
                )

...
class LULDCalculator:
    ...
    def calculate_luld_bands(
        reference_price: float, tier: int = 1
    ) -> Tuple[float, float]:
...
class DistanceToLULDUpFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        lower_band, upper_band = self.luld_calc.calculate_luld_bands(
            reference_price, tier=2
        )
    ...
    def get_requirements(self) -> Dict[str, Any]:
        """Requires price data."""
        return {"current": {"fields": ["current_price", "prev_day_close"]}}

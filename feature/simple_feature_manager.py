...
class SimpleFeatureManager:
    ...
    def __init__(
        self, symbol: str, config: Any, logger: Optional[logging.Logger] = None
    ):
        self.symbol = symbol
        self.config = config
    ...
    def _create_hf_features(self) -> List[BaseFeature]:
        ...
        try:
            ...
            from .hf.price_features import (
                PriceVelocityFeature,
                PriceAccelerationFeature,
            )
            from .hf.tape_features import (
                TapeImbalanceFeature,
                TapeAggressionRatioFeature,
            )
            from .hf.quote_features import (
                SpreadCompressionFeature,
                QuoteVelocityFeature,
                QuoteImbalanceFeature,
            )
            from .hf.volume_features import (
                VolumeVelocityFeature,
    ...
    def _create_mf_features(self) -> List[BaseFeature]:
        ...
        try:
            ...
            from .mf.candle_features import (
                ...
                PositionInCurrentCandle5mFeature,
                BodySizeRelative1mFeature,
                BodySizeRelative5mFeature,
            )
            from .mf.candle_analysis_features import (
                PositionInPreviousCandle1mFeature,
                PositionInPreviousCandle5mFeature,
                UpperWickRelative1mFeature,
                LowerWickRelative1mFeature,
                UpperWickRelative5mFeature,
                LowerWickRelative5mFeature,
            )
            from .mf.ema_features import (
                DistanceToEMA9_1mFeature,
                DistanceToEMA20_1mFeature,
                DistanceToEMA9_5mFeature,
                DistanceToEMA20_5mFeature,
            )
            from .mf.ema_features_v2 import (
                EMAInteractionPatternFeature,
                EMACrossoverDynamicsFeature,
                EMATrendAlignmentFeature,
            )
            from .mf.swing_features import (
                SwingHighDistance1mFeature,
                SwingLowDistance1mFeature,
                SwingHighDistance5mFeature,
                SwingLowDistance5mFeature,
            )
            from .mf.velocity_features import (
                PriceVelocity1mFeature,
                PriceVelocity5mFeature,
                VolumeVelocity1mFeature,
                VolumeVelocity5mFeature,
            )
            from .mf.acceleration_features import (
                PriceAcceleration1mFeature,
                PriceAcceleration5mFeature,
                VolumeAcceleration1mFeature,
                VolumeAcceleration5mFeature,
            )
            from .volume_analysis.vwap_features import (
                DistanceToVWAPFeature,
                VWAPSlopeFeature,
                PriceVWAPDivergenceFeature,
            )
            from .volume_analysis.vwap_features_v2 import (
                VWAPInteractionDynamicsFeature,
                VWAPBreakoutQualityFeature,
                VWAPMeanReversionTendencyFeature,
            )
            from .volume_analysis.relative_volume_features import (
                RelativeVolumeFeature,
                VolumeSurgeFeature,
                CumulativeVolumeDeltaFeature,
                VolumeMomentumFeature,
            )
            from .professional.ta_features import (
                ProfessionalEMASystemFeature,
                ProfessionalVWAPAnalysisFeature,
                ProfessionalMomentumQualityFeature,
                ProfessionalVolatilityRegimeFeature,
            )
            from .sequence_aware.sequence_features import (
                TrendAccelerationFeature,
                VolumePatternEvolutionFeature,
                MomentumQualityFeature,
                PatternMaturationFeature,
            )
    ...
    def _create_lf_features(self) -> List[BaseFeature]:
        ...
        try:
            ...
            from .market_structure.luld_features import (
                DistanceToLULDUpFeature,
                DistanceToLULDDownFeature,
                LULDBandWidthFeature,
            )
            from .context.context_features import (
                SessionProgressFeature,
                MarketStressFeature,
                SessionVolumeProfileFeature,
            )
            from .adaptive.adaptive_features import (
    ...
    def calculate_features(
        self, market_data: Dict[str, Any], category: str
    ) -> Dict[str, float]:
        ...
        total_features = len(
            [f for f in self._feature_collections[category] if f.enabled]
        )
        feature_idx = 0
        ...
        for feature in self._feature_collections[category]:
            ...
            feature_idx += 1
            self.logger.debug(
                f"DEBUG: Calculating {category} feature {feature_idx}/{total_features}: {feature.name}"
            )
            ...
            try:
                ...
                if value is None or np.isnan(value) or np.isinf(value):
                    self.logger.warning(
                        f"Feature {feature.name} returned invalid value ({value}), using 0.0"
                    )
            ...
            self.logger.debug(
                f"DEBUG: Feature {feature.name} calculated: {results.get(feature.name, 'ERROR')}"
            )

        return results

    def vectorize_features(
        self, features: Dict[str, float], category: str
    ) -> np.ndarray:
        ...
        for feature in self._feature_collections[category]:
            if feature.name in features:
                ...
                if value is None or np.isnan(value) or np.isinf(value):
                    self.logger.warning(
                        f"Feature {feature.name} has invalid value ({value}) during vectorization, using 0.0"
                    )
                ...
                self.logger.warning(
                    f"Feature {feature.name} not found in results for category {category}, using 0.0"
                )
                ordered_features.append(0.0)
    ...
    def extract_features(
        ...
        try:
            ...
            for category in ["hf", "mf", "lf"]:
                ...
                self.logger.debug(
                    f"DEBUG: {category} features extracted, shape: {features.shape if features is not None else 'None'}"
                )
            ...
            for feature in category_features:
                ...
                if data_type != "unknown" and data_type != "current":
                    if data_type not in requirements:
                        requirements[data_type] = {"lookback": 0, "fields": set()}

                    if "lookback" in feat_req:
                        requirements[data_type]["lookback"] = max(
                            requirements[data_type]["lookback"], feat_req["lookback"]
                        )

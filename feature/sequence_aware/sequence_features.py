...
class TrendAccelerationFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        early_velocity = (
            (early_segment[-1] - early_segment[0]) / len(early_segment)
            if early_segment[0] > 0
            else 0
        )
        late_velocity = (
            (late_segment[-1] - late_segment[0]) / len(late_segment)
            if late_segment[0] > 0
            else 0
        )
        ...
        if early_segment[0] > 0:
            ...
            late_velocity_norm = (
                late_velocity / early_segment[0] if len(prices) >= 20 else 0
            )
...
class MomentumQualityFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        direction_changes = sum(
            1
            for i in range(1, len(move_directions))

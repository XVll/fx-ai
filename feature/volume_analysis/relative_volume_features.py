...
class RelativeVolumeFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        historical_volumes = [
            bar.get("volume", 0) for bar in bars[-30:]
        ]  # Last 30 minutes
    ...
    def get_requirements(self) -> Dict[str, Any]:
        ...
        return {"mf_bars_1m": {"lookback": 30, "fields": ["volume"]}}
...
class VolumeSurgeFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        recent_volumes = [
            bar.get("volume", 0) for bar in bars[-10:-1]
        ]  # Previous 9 bars
    ...
    def get_requirements(self) -> Dict[str, Any]:
        """Requires 1m volume data."""
        return {"mf_bars_1m": {"lookback": 10, "fields": ["volume"]}}
...
class CumulativeVolumeDeltaFeature(BaseFeature):
    ...
    def _is_buy_trade(self, trade: Dict[str, Any], window_data: Dict[str, Any]) -> bool:
        ...
        if quotes:
            last_quote = quotes[-1]
            mid = (
                last_quote.get("bid_price", price) + last_quote.get("ask_price", price)
            ) / 2
            return price >= mid
    ...
    def get_requirements(self) -> Dict[str, Any]:
        ...
        return {"hf_data_window": {"lookback": 30, "fields": ["trades", "quotes"]}}
...
class VolumeMomentumFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        recent_change = (
            (volumes[-1] - volumes[-2]) / max(1, volumes[-2])
            if len(volumes) >= 2
            else 0
        )
        older_change = (
            (volumes[-2] - volumes[-3]) / max(1, volumes[-3])
            if len(volumes) >= 3
            else 0
        )
    ...
    def get_requirements(self) -> Dict[str, Any]:
        """Requires 1m volume data."""
        return {"mf_bars_1m": {"lookback": 5, "fields": ["volume"]}}

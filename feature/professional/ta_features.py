...
class ProfessionalEMASystemFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            alignment_direction = 1 if recent_data['ema9'].iloc[-1] > recent_data['ema20'].iloc[-1] else -1
            
            # 2. EMA slope strength (trending vs flat)
            ema9_slope = (recent_data['ema9'].iloc[-1] - recent_data['ema9'].iloc[0]) / recent_data['ema9'].iloc[0]
            ema20_slope = (recent_data['ema20'].iloc[-1] - recent_data['ema20'].iloc[0]) / recent_data['ema20'].iloc[0]
...
class ProfessionalVWAPAnalysisFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            vwap_trend = vwap_returns.rolling(window=5).mean().iloc[-1] if len(vwap_returns) >= 5 else 0
            ...
            if volume_above > 0 and volume_below > 0:
                volume_bias = (volume_above - volume_below) / (volume_above + volume_below)
            
            # Combine VWAP metrics
            current_position = relative_position.iloc[-1] if not relative_position.empty else 0
...
class ProfessionalMomentumQualityFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            macd_momentum = 1.0 if macd_value > macd_signal and macd_histogram > 0 else -1.0
            ...
            avg_volume = recent['volume_sma'].iloc[-1] if not pd.isna(recent['volume_sma'].iloc[-1]) else recent_volume
            volume_confirmation = min(recent_volume / (avg_volume + 1e-8), 2.0) / 2.0  # Normalize to [0,1]
...
class ProfessionalVolatilityRegimeFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            bb_trend = (recent['bb_width'].iloc[-1] - recent['bb_width'].iloc[0]) / recent['bb_width'].iloc[0]
            atr_trend = (recent['atr'].iloc[-1] - recent['atr'].iloc[0]) / recent['atr'].iloc[0]
    ...
    def get_requirements(self) -> Dict[str, Any]:
        return {'data_type': 'mf_data', 'lookback': 20, 'fields': ['close', 'high', 'low']}
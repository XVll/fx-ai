...
class MarketStressFeature(BaseFeature):
    ...
    def calculate_raw(self, market_data: Dict[str, Any]) -> float:
        ...
        recent_volatility = (
            np.mean(price_changes[-10:])
            if len(price_changes) >= 10
            else np.mean(price_changes)
        )
        ...
        historical_avg = (
            np.mean(historical_volumes) if historical_volumes else avg_volume
        )

        volume_stress = 0.0
        if historical_avg > 0:
            volume_stress = (
                min(2.0, avg_volume / historical_avg) / 2.0
            )  # Normalize to [0,1]

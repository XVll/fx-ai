...
class ComponentMetrics:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if not self.values:
            return {"name": self.name, "type": self.type, "triggered": False}
        ...
        stats = {
            ...
            # Sign distribution
            "positive_rate": self.count_positive / self.count_triggered
            if self.count_triggered > 0
            else 0,
            "negative_rate": self.count_negative / self.count_triggered
            if self.count_triggered > 0
            else 0,
            "zero_rate": self.count_zero / self.count_triggered
            if self.count_triggered > 0
            else 0,
            # Magnitude analysis
            "mean_positive": np.mean([v for v in values_array if v > 0])
            if self.count_positive > 0
            else 0,
            "mean_negative": np.mean([v for v in values_array if v < 0])
            if self.count_negative > 0
            else 0,
            # Timing
            "mean_trigger_interval": np.mean(self.trigger_intervals)
            if self.trigger_intervals
            else None,
            # Action correlations
            "mean_when_buy": np.mean(self.when_action_buy)
            if self.when_action_buy
            else None,
            "mean_when_sell": np.mean(self.when_action_sell)
            if self.when_action_sell
            else None,
            "mean_when_hold": np.mean(self.when_action_hold)
            if self.when_action_hold
            else None,
            # Profitability correlations
            "mean_when_profitable": np.mean(self.when_profitable)
            if self.when_profitable
            else None,
...
class RewardMetricsTracker:
    ...
    def update_component(
        ...
        if name not in self.component_metrics:
            self.logger.warning(
                f"Component {name} not registered, skipping metrics update"
            )
            return
    ...
    def end_episode(self, final_portfolio_metrics: Dict[str, Any]):
        ...
        episode_summary = {
            ...
            "mean_reward_per_step": self.episode_total_reward
            / max(1, self.current_episode_steps),
            # Component breakdown
        ...
        for name, metrics in self.component_metrics.items():
            ...
            if self.current_episode_steps > 0 and stats.get("triggered", False):
                stats["trigger_rate"] = (
                    stats["count_triggered"] / self.current_episode_steps
                )
            else:
        ...
        dominant_positive = max(
            self.episode_component_totals.items(), key=lambda x: x[1], default=(None, 0)
        )
        dominant_negative = min(
            self.episode_component_totals.items(), key=lambda x: x[1], default=(None, 0)
        )

        episode_summary["dominant_positive_component"] = (
            {"name": dominant_positive[0], "total": dominant_positive[1]}
            if dominant_positive[0]
            else None
        )

        episode_summary["dominant_negative_component"] = (
            {"name": dominant_negative[0], "total": dominant_negative[1]}
            if dominant_negative[0]
            else None
        )
    ...
    def get_component_analysis(self) -> Dict[str, Any]:
        ...
        component_aggregates = defaultdict(
            lambda: {
                "total_value": 0.0,
                "mean_values": [],
                "trigger_rates": [],
        ...
        for comp_name, agg in component_aggregates.items():
            if agg["mean_values"]:
                component_analysis[comp_name] = {
                    ...
                    "consistency": 1.0
                    - np.std(agg["mean_values"]) / (np.mean(agg["mean_values"]) + 1e-8),
                }
        ...
        for comp_name, comp_analysis in component_analysis.items():
            ...
            if abs(comp_analysis["global_total_value"]) > abs(
                analysis.get("total_reward", 0) * 0.5
            ):
                problems.append(
                    {
                        "component": comp_name,
                        "issue": "dominant",
                        "severity": "high",
                        "description": "Component contributes >50% of total reward",
                    }
                )
                ...
                problems.append(
                    {
                        ...
                        "severity": "medium",
                        "description": "Component is negative >90% of the time",
                    }
                )
                ...
                problems.append(
                    {
                        ...
                        "severity": "low",
                        "description": "Component triggers <1% of the time",
                    }
                )
    ...
    def get_correlation_analysis(self) -> Dict[str, Any]:
        ...
        correlations = {
            "reward_vs_trades": np.corrcoef(episode_rewards, episode_trades)[0, 1]
            if len(episode_rewards) > 1
            else None,
            "reward_vs_win_rate": np.corrcoef(episode_rewards, episode_win_rates)[0, 1]
            if len(episode_rewards) > 1
            else None,
        }
        ...
        for comp_name in self.component_metrics.keys():
            comp_totals = [
                ep["component_totals"].get(comp_name, 0) for ep in self.episode_metrics
            ]

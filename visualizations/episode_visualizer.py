...
class EpisodeVisualizer:
    ...
    def create_episode_chart(self) -> Optional[wandb.Image]:
        ...
        try:
            ...
            return wandb.Image(img, caption=f"Episode {self.current_episode_info.get('episode_num', 'N/A')}")
    ...
    def _plot_price_chart(self, ax, df):
        ...
        if 'sma_fast' in df.columns:
            ax.plot(df.index, df['sma_fast'], 'b-', linewidth=1, alpha=0.5, label='Fast MA')
        if 'sma_slow' in df.columns:
            ax.plot(df.index, df['sma_slow'], 'r-', linewidth=1, alpha=0.5, label='Slow MA')
        ...
        for trade in self.trades:
            ...
            if trade['action'] == 'buy':
                ax.scatter(idx, price, color='green', s=100, marker='^', 
                ...
                ax.scatter(idx, price, color=color, s=100, marker='v', 
        ...
        ax.set_title(f"Trading Episode - {self.current_episode_info.get('symbol', 'N/A')} "
                    f"({self.current_episode_info.get('date', 'N/A')})", fontsize=12, weight='bold')
    ...
    def _plot_volume(self, ax, df):
        ...
        colors = ['g' if i == 0 or df['price'].iloc[i] >= df['price'].iloc[i-1] else 'r' 
    ...
    def _plot_rewards(self, ax, df):
        ...
        ax.fill_between(df.index, 0, cumulative_reward, 
    ...
    def _plot_indicators(self, ax, df):
        ...
        if 'volatility' in df.columns:
            ax.plot(df.index, df['volatility'], 'orange', linewidth=1, alpha=0.7, label='Volatility')
    ...
    def _add_statistics(self, ax, df):
        ...
        stats_text += f"Episode: {self.current_episode_info.get('episode_num', 'N/A')}\n\n"
        ...
        # Market stats
        if len(df) > 0:
            stats_text += f"Price Range: ${df['price'].min():.4f} - ${df['price'].max():.4f}\n"
    ...
    def _create_feature_chart(self, df) -> Optional[wandb.Image]:
        ...
        try:
            ...
            feature_cols = [col for col in df.columns if col.startswith(('momentum', 'volume_', 'rsi', 'volatility'))]
    ...
    def _create_action_distribution_chart(self, df) -> Optional[wandb.Image]:
        ...
        try:
            ...
            # Action over time
            action_numeric = df['action'].map({'hold': 0, 'buy': 1, 'sell': -1}).fillna(0)
    ...
    def _create_reward_components_chart(self, df) -> Optional[wandb.Image]:
        ...
        try:
            reward_cols = [col for col in df.columns if 'reward' in col.lower() and col != 'reward']
            ...
            for col in reward_cols:
                ax.fill_between(df.index, bottom, bottom + df[col], label=col, alpha=0.7)
                bottom += df[col]

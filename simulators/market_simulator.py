...
class MarketSimulator:
    ...
    def __init__(
        ...
        if feature_manager is None:
            self.feature_manager = SimpleFeatureManager(
                symbol=symbol, config=model_config, logger=self.logger
            )
        else:
    ...
    def initialize_day(self, date: datetime) -> bool:
        ...
        try:
            self.logger.info(
                f"Initializing market simulator for {self.symbol} on {date}"
            )
            ...
            if self.current_date is not None and self.current_date != new_cache_date:
                self.logger.info(
                    f"Switching from {self.current_date} to {new_cache_date}, unloading cache session"
                )
                self.feature_cache_manager.unload_session()

            # Load feature cache session for this day
            if not self.feature_cache_manager.load_session(
                self.symbol, new_cache_date, self.feature_manager
            ):
                self.logger.warning(
                    f"Failed to load feature cache session for {self.symbol} on {new_cache_date}"
                )
            ...
            if cache_key in self._precomputed_cache:
                self.logger.info(
                    f"Using cached precomputed states for {self.symbol} on {date}"
                )
            ...
            self.logger.info(
                f"Successfully initialized {len(self.df_market_state)} market states with warmup data"
            )
            return True
    ...
    def _load_previous_day_full_data(self, date: datetime) -> Dict[str, pd.DataFrame]:
        ...
        has_prev_data = any(
            df is not None and not df.empty for df in prev_data_from_cache.values()
        )
        ...
        if prev_date is None:
            self.logger.warning(
                "No previous trading day found - starting without warmup data"
            )
            return {}
    ...
    def _combine_data_with_warmup(
        self, current_data: Dict[str, pd.DataFrame], prev_data: Dict[str, pd.DataFrame]
    ):
        ...
        if not self.combined_trades.empty:
            ...
            self.combined_bars_1s = self._build_1s_bars_from_trades(
                self.combined_trades, full_timeline
            )
            ...
            self.combined_bars_1s = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume", "trade_count"]
            )
            self.combined_bars_1m = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume", "trade_count"]
            )
            self.combined_bars_5m = pd.DataFrame(
                columns=["open", "high", "low", "close", "volume", "trade_count"]
            )

        if not self.combined_bars_1s.empty:
            self.logger.info(
                f"Combined data spans {len(self.combined_bars_1s)} seconds with warmup"
            )
        else:
            self.logger.warning(
                "No warmup data available - will use synthetic data for early hours"
            )
    ...
    def _build_timeline(self, date: datetime) -> pd.DatetimeIndex:
        ...
        start_et = (
            pd.Timestamp(date_obj)
            .tz_localize(self.market_tz)
            .replace(
    ...
    def _extract_features_batch(
        batch_data: List[Tuple[int, pd.Timestamp, pd.Series]], shared_data: Dict
        ...
        for idx, timestamp, row in batch_data:
            ...
            hf_window = MarketSimulator._build_hf_window_for_batch(
                timestamp, shared_data
            )
            mf_window = MarketSimulator._build_mf_window_for_batch(
                timestamp, shared_data
            )
            lf_window = MarketSimulator._build_lf_window_for_batch(
                timestamp, shared_data
            )
            ...
            hf_feat = features.get(
                "hf", np.zeros((shared_data["hf_seq_len"], shared_data["hf_feat_dim"]))
            )
            mf_feat = features.get(
                "mf", np.zeros((shared_data["mf_seq_len"], shared_data["mf_feat_dim"]))
            )
            lf_feat = features.get(
                "lf", np.zeros((shared_data["lf_seq_len"], shared_data["lf_feat_dim"]))
            )
    ...
    @staticmethod
    def _build_hf_window_for_batch(
        current_ts: pd.Timestamp, shared_data: Dict
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(window_size):
            ...
            if not combined_trades.empty:
                mask = (combined_trades.index >= ts) & (
                    combined_trades.index < ts + pd.Timedelta(seconds=1)
                )
            ...
            if not combined_quotes.empty:
                mask = (combined_quotes.index >= ts) & (
                    combined_quotes.index < ts + pd.Timedelta(seconds=1)
                )
    ...
    @staticmethod
    def _build_mf_window_for_batch(
        current_ts: pd.Timestamp, shared_data: Dict
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _build_lf_window_for_batch(
        current_ts: pd.Timestamp, shared_data: Dict
    ) -> List[Dict[str, Any]]:
    ...
    def _precompute_states(
        self, timeline: pd.DatetimeIndex, day_data: Dict[str, pd.DataFrame]
    ) -> pd.DataFrame:
        ...
        """
        self.logger.info(
            f"Starting vectorized pre-computation of {len(timeline)} market states..."
        )
        ...
        if not quotes_df.empty:
            ...
            df_states.loc[mask, "best_bid"] = quotes_reindexed.loc[
                mask, "bid_price"
            ].fillna(df_states["best_bid"])
            df_states.loc[mask, "best_ask"] = quotes_reindexed.loc[
                mask, "ask_price"
            ].fillna(df_states["best_ask"])
            df_states.loc[mask, "bid_size"] = quotes_reindexed.loc[
                mask, "bid_size"
            ].fillna(df_states["bid_size"])
            df_states.loc[mask, "ask_size"] = quotes_reindexed.loc[
                mask, "ask_size"
            ].fillna(df_states["ask_size"])
        ...
        # Fix invalid spreads
        invalid_spread = (df_states["best_bid"] >= df_states["best_ask"]) | (
            df_states["best_bid"] <= 0
        )
        df_states.loc[invalid_spread, "spread"] = (
            df_states.loc[invalid_spread, "current_price"] * 0.001
        )
        df_states.loc[invalid_spread, "best_bid"] = (
            df_states.loc[invalid_spread, "current_price"]
            - df_states.loc[invalid_spread, "spread"] / 2
        )
        df_states.loc[invalid_spread, "best_ask"] = (
            df_states.loc[invalid_spread, "current_price"]
            + df_states.loc[invalid_spread, "spread"] / 2
        )
        ...
        df_states["market_session"] = df_states.index.to_series().apply(
        ...
        if not self.combined_bars_1s.empty:
            ...
            df_states["session_value"] = (
                bars_in_day["close"] * bars_in_day["volume"]
            ).cumsum()
            df_states["session_vwap"] = df_states["session_value"] / df_states[
                "session_volume"
            ].replace(0, 1)
            df_states["intraday_high"] = (
                bars_in_day["high"].expanding().max().fillna(last_price)
            )
            df_states["intraday_low"] = (
                bars_in_day["low"].expanding().min().fillna(last_price)
            )
        else:
        ...
        placeholder_memory_mb = (num_states * 100 * 4) / (
            1024 * 1024
        )  # Small placeholder overhead

        self.logger.info("Feature dimensions per timestamp (computed on-demand):")
        self.logger.info(
            f"  - HF: {self.model_config.hf_seq_len}x{self.model_config.hf_feat_dim} = {self.model_config.hf_seq_len * self.model_config.hf_feat_dim} values"
        )
        self.logger.info(
            f"  - MF: {self.model_config.mf_seq_len}x{self.model_config.mf_feat_dim} = {self.model_config.mf_seq_len * self.model_config.mf_feat_dim} values"
        )
        self.logger.info(
            f"  - LF: {self.model_config.lf_seq_len}x{self.model_config.lf_feat_dim} = {self.model_config.lf_seq_len * self.model_config.lf_feat_dim} values"
        )
        # Static features have been moved to lf branch
        self.logger.info(
            f"Placeholder memory usage: {placeholder_memory_mb:.1f} MB for {num_states} states"
        )
        self.logger.info(
            f"Completed FAST initialization of {len(df_states)} market states (features computed on-demand)"
        )
        return df_states

    def _build_1s_bars_from_trades(
        self, trades_df: pd.DataFrame, timeline: pd.DatetimeIndex
    ) -> pd.DataFrame:
    ...
    def _get_quote_at_time(
        self, quotes_df: pd.DataFrame, timestamp: pd.Timestamp
    ...
    def _is_halted_at_time(
        self, status_df: pd.DataFrame, timestamp: pd.Timestamp
    ) -> bool:
    ...
    def _build_hf_window_with_warmup(
        self, current_ts: pd.Timestamp
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(window_size):
            ...
            second_trades = []
            if not self.combined_trades.empty:
                mask = (self.combined_trades.index >= ts) & (
                    self.combined_trades.index < ts + pd.Timedelta(seconds=1)
                )
            ...
            if not self.combined_quotes.empty:
                mask = (self.combined_quotes.index >= ts) & (
                    self.combined_quotes.index < ts + pd.Timedelta(seconds=1)
                )
    ...
    def _build_mf_window_with_warmup(
        self, current_ts: pd.Timestamp
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(window_size):
            ...
            if (
                not self.combined_bars_1m.empty
                and bar_ts in self.combined_bars_1m.index
            ):
                ...
                if window:
                    ...
                    prev_bars = self.combined_bars_1m[
                        self.combined_bars_1m.index < bar_ts
                    ]
                    if not prev_bars.empty:
                        last_price = prev_bars.iloc[-1]["close"]
                ...
                if last_price == 0:
                    ...
                    if not self.combined_bars_1s.empty:
                        ...
                        first_prices = self.combined_bars_1s[
                            self.combined_bars_1s["close"] > 0
                        ]
    ...
    def _build_lf_window_with_warmup(
        self, current_ts: pd.Timestamp
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(window_size):
            ...
            if (
                not self.combined_bars_5m.empty
                and bar_ts in self.combined_bars_5m.index
            ):
                ...
                if window:
                    ...
                    # Find last known price before this timestamp
                    prev_bars = self.combined_bars_5m[
                        self.combined_bars_5m.index < bar_ts
                    ]
                    if not prev_bars.empty:
                        last_price = prev_bars.iloc[-1]["close"]
    ...
    def get_market_state(
        ...
        if timestamp is not None:
            ...
            if timestamp in self.df_market_state.index:
                ...
                valid_times = self.df_market_state.index[
                    self.df_market_state.index <= timestamp
                ]
    ...
    def get_current_features(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        if self.df_market_state is None or self.current_index >= len(
            self.df_market_state
        ):
            return None
        ...
        if not row.get("features_computed", False):
            ...
            if cached_features is not None:
                ...
                self.logger.debug(
                    f"DEBUG: Loading features from persistent cache for {timestamp}"
                )

                # Update the DataFrame with cached features
                self.df_market_state.at[timestamp, "hf_features"] = cached_features.get(
                    "hf",
                    np.zeros(
                        (self.model_config.hf_seq_len, self.model_config.hf_feat_dim)
                    ),
                )
                self.df_market_state.at[timestamp, "mf_features"] = cached_features.get(
                    "mf",
                    np.zeros(
                        (self.model_config.mf_seq_len, self.model_config.mf_feat_dim)
                    ),
                )
                self.df_market_state.at[timestamp, "lf_features"] = cached_features.get(
                    "lf",
                    np.zeros(
                        (self.model_config.lf_seq_len, self.model_config.lf_feat_dim)
                    ),
            ...
            try:
                self.logger.debug(
                    f"DEBUG: Computing features on-demand for {timestamp}"
                )
                ...
                # Extract features
                self.logger.debug(
                    f"DEBUG: About to extract features for timestamp {timestamp}"
                )
                features = self.feature_manager.extract_features(context)
                self.logger.debug("DEBUG: Features extracted successfully")
                ...
                self.df_market_state.at[timestamp, "hf_features"] = features.get(
                    "hf",
                    np.zeros(
                        (self.model_config.hf_seq_len, self.model_config.hf_feat_dim)
                    ),
                )
                self.df_market_state.at[timestamp, "mf_features"] = features.get(
                    "mf",
                    np.zeros(
                        (self.model_config.mf_seq_len, self.model_config.mf_feat_dim)
                    ),
                )
                self.df_market_state.at[timestamp, "lf_features"] = features.get(
                    "lf",
                    np.zeros(
                        (self.model_config.lf_seq_len, self.model_config.lf_feat_dim)
                    ),
                ...
                self.logger.error(
                    f"On-demand feature computation failed for {timestamp}: {e}"
                )

...
class ModelInternalsCollector(MetricCollector):
    ...
    def __init__(
        ...
        self.attribution_analysis_interval = (
            0  # No time-based limits (controlled by PPO agent)
        )
            ...
            try:
                ...
                if model_config:
                    for branch in feature_names.keys():
                        ...
                        if hasattr(model_config, seq_len_attr) and hasattr(
                            model_config, feat_dim_attr
                        ):
                ...
                self.logger.info(
                    "✅ Comprehensive SHAP feature attribution analyzer initialized"
                )
    ...
    def _register_metrics(self):
        ...
        if self.enable_attribution:
            ...
            for branch in ["hf", "mf", "lf", "portfolio"]:
                self.register_metric(
                    f"top_feature_importance_{branch}",
                    MetricMetadata(
                        category=MetricCategory.MODEL,
                        metric_type=MetricType.GAUGE,
    ...
    def collect(self) -> Dict[str, MetricValue]:
        ...
        try:
            ...
            if self.last_attention_weights is not None:
                ...
                metrics[f"{self.category.value}.{self.name}.attention_entropy"] = (
                    MetricValue(entropy)
                )
                ...
                metrics[f"{self.category.value}.{self.name}.attention_max_weight"] = (
                    MetricValue(max_weight)
                )
                metrics[f"{self.category.value}.{self.name}.attention_focus_branch"] = (
                    MetricValue(focus_branch)
                )

            # Action probability metrics
            if self.last_action_probs is not None:
                if (
                    isinstance(self.last_action_probs, tuple)
                    and len(self.last_action_probs) == 2
                ):
                    ...
                    metrics[
                        f"{self.category.value}.{self.name}.action_type_entropy"
                    ] = MetricValue(type_entropy)

                    # Size entropy
                    size_entropy = -np.sum(size_probs * np.log(size_probs + 1e-8))
                    metrics[
                        f"{self.category.value}.{self.name}.action_size_entropy"
                    ] = MetricValue(size_entropy)

                    # Combined entropy (average)
                    combined_entropy = (type_entropy + size_entropy) / 2
                    metrics[f"{self.category.value}.{self.name}.action_entropy"] = (
                        MetricValue(combined_entropy)
                    )

                    # Confidence (max probability)
                    confidence = max(np.max(type_probs), np.max(size_probs)) * 100
                    metrics[f"{self.category.value}.{self.name}.action_confidence"] = (
            ...
            for branch, stats in self.feature_stats.items():
                if stats:
                    metrics[
                        f"{self.category.value}.{self.name}.feature_{branch}_mean"
                    ] = MetricValue(stats["mean"])
                    metrics[
                        f"{self.category.value}.{self.name}.feature_{branch}_std"
                    ] = MetricValue(stats["std"])
                    metrics[
                        f"{self.category.value}.{self.name}.feature_{branch}_sparsity"
                    ] = MetricValue(stats["sparsity"])
                ...
                if (
                    self.last_attention_weights is not None
                    and len(self.attention_weights_history) > 1
                ):
                    try:
                        ...
                        if len(recent_weights) > 1:
                            ...
                            metrics[
                                f"{self.category.value}.{self.name}.attention_stability"
            ...
            if (
                hasattr(self, "last_attribution_metrics")
                and self.last_attribution_metrics
            ):
                for metric_name, value in self.last_attribution_metrics.items():
                    metrics[f"{self.category.value}.{self.name}.{metric_name}"] = (
                        MetricValue(value)
                    )
    ...
    def update_state_for_attribution(self, state_dict: Dict[str, torch.Tensor]):
        ...
        if self.enable_attribution:
            ...
            if len(self.state_buffer) % 10 == 0:
                self.logger.debug(
                    f"Attribution state buffer: {len(self.state_buffer)} states stored"
                )
    ...
    def run_periodic_shap_analysis(self) -> Optional[Dict[str, Any]]:
        ...
        if (
            hasattr(self.attribution_analyzer, "config")
            and not self.attribution_analyzer.config.enabled
        ):
            return None
        ...
        self.logger.debug(
            f"SHAP check: {time_since_last:.1f}s since last, {states_available} states available"
        )

        if states_available < 2:  # Need minimal states for analysis
            self.logger.debug(
                f"Attribution skipped: only {states_available} states (need 2)"
            )
            return None

        # Setup background data if needed
        if (
            not hasattr(self.attribution_analyzer, "background_cache")
            or self.attribution_analyzer.background_cache is None
        ):
            background_states = list(self.state_buffer)[
                :10
            ]  # Use first 10 states as background
            self.attribution_analyzer.setup_background(background_states)

        # Run comprehensive SHAP analysis
        try:
            self.logger.info(
                f"🔍 Starting comprehensive SHAP analysis with {states_available} states"
            )
            ...
            if results and "error" not in results:
                ...
                if hasattr(self.attribution_analyzer, "get_summary_for_logging"):
                    dashboard_summary = (
                        self.attribution_analyzer.get_summary_for_logging()
                    )
    ...
    def configure_shap_frequency(self, update_frequency: int):
        ...
        if self.attribution_analyzer and hasattr(self.attribution_analyzer, "config"):
            ...
            self.logger.info(
                f"Updated SHAP frequency from {old_freq} to {update_frequency} updates"
            )
        ...
        try:
            ...
            if not hasattr(self, "_simple_analyzer"):
                self._simple_analyzer = SimpleFeatureAttribution(
                    ...
                    feature_names=self.attribution_analyzer.feature_names
                    if self.attribution_analyzer
                    else {},
                    branch_configs=self.attribution_analyzer.branch_configs
                    if self.attribution_analyzer
                    else {},
                    logger=self.logger,
                )
            ...
            analysis_states = list(self.state_buffer)[
                -3:
            ]  # Use 3 states for gradient analysis

            self.logger.info(
                f"🚀 Running fast gradient attribution on {len(analysis_states)} states"
            )
            start_time = time.time()

            # Run simple attribution
            attribution_results = self._simple_analyzer.analyze_features(
                analysis_states
            )
            ...
            if attribution_results and "error" not in attribution_results:
                self.logger.info(
                    f"✅ Fast gradient attribution completed in {analysis_time:.2f}s"
                )
                ...
                if "input_gradients" in attribution_results:
                    ...
                    simplified_results = {
                        ...
                        "summary_stats": {
                            "mean_absolute_importance": float(
                                np.mean([abs(v) for v in branch_importance.values()])
                            ),
                    ...
                    branch_ranking = sorted(
                        branch_importance.items(), key=lambda x: abs(x[1]), reverse=True
                    )
                    self.logger.info(
                        f"📊 Branch ranking: {[(b, f'{s:.3f}') for b, s in branch_ranking]}"
                    )

                    top_5 = simplified_results["top_features"][:5]
                    feature_list = [
                        (f["branch"], f"{f['importance']:.4f}") for f in top_5
                    ]
                    ...
                    try:
                        ...
                        if wandb.run:
                            ...
                            for branch, importance in branch_importance.items():
                                log_dict[f"attribution/branch_{branch}_importance"] = (
                                    importance
                                )

                            # Summary stats
                            for stat, value in simplified_results[
                                "summary_stats"
                            ].items():
                            ...
                            for i, feature_info in enumerate(
                                simplified_results["top_features"][:5]
                            ):
                                log_dict[
                                    f"attribution/top_branch_{i + 1}_importance"
                                ] = feature_info["importance"]
                                log_dict[f"attribution/top_branch_{i + 1}_name"] = (
                                    feature_info["branch"]
                                )

                            # Analysis metadata
                            log_dict["attribution/analysis_time"] = analysis_time
                            log_dict["attribution/n_samples_analyzed"] = len(
                                analysis_states
                            )
                            ...
                            bar_data = [
                                [branch, importance]
                                for branch, importance in zip(branches, importances)
                            ]
                            bar_table = wandb.Table(
                                data=bar_data, columns=["Branch", "Importance"]
                            )
                            log_dict["attribution/branch_importance_chart"] = (
                                wandb.plot.bar(
                                    bar_table,
                                    "Branch",
                                    "Importance",
                            ...
                            log_dict["attribution/branch_importance_line"] = (
                                wandb.plot.line(
                                    bar_table,
                                    "Branch",
                                    "Importance",
                            ...
                            try:
                                ...
                                log_dict["attribution/branch_pie_chart"] = wandb.Html(
                                    pio.to_html(fig, include_plotlyjs="inline")
                                )
                            ...
                            for i, (branch, importance) in enumerate(
                                ...
                                summary_data.append(
                                    [
                                        ...
                                        branch,
                                        f"{importance:.4f}",
                                        f"{abs(importance) / sum(abs(v) for v in importances) * 100:.1f}%",
                                    ]
                                )
                            ...
                            wandb.log(log_dict)
                            self.logger.info(
                                f"📈 Logged {len(log_dict)} gradient attribution metrics + native WandB plots"
                            )
                    except Exception as e:
    ...
    def _store_attribution_results(self, attribution_results: Dict[str, Any]):
        ...
        if "branch_importance" in attribution_results:
            for branch, importance in attribution_results["branch_importance"].items():
                self.last_attribution_metrics[f"shap_branch_{branch}_importance"] = (
                    importance
                )
        ...
        if "top_features" in attribution_results:
            for i, feature_info in enumerate(
                attribution_results["top_features"][:5]
            ):  # Top 5
                self.last_attribution_metrics[
                    f"shap_top_feature_{i + 1}_importance"
                ] = feature_info["importance"]
                # Handle both SHAP and gradient attribution formats
                if "feature_name" in feature_info:
                    self.last_attribution_metrics[f"shap_top_feature_{i + 1}_name"] = (
                        f"{feature_info['branch']}.{feature_info['feature_name']}"
                    )
                else:
        ...
        if "dead_features" in attribution_results:
            total_dead = sum(
                len(features)
                for features in attribution_results["dead_features"].values()
            )
            ...
            for branch, dead_list in attribution_results["dead_features"].items():
                self.last_attribution_metrics[f"shap_dead_features_{branch}"] = len(
                    dead_list
                )

        # Store analysis metadata
        self.last_attribution_metrics["shap_analysis_time"] = attribution_results.get(
            "analysis_time", 0
        )
        self.last_attribution_metrics["shap_samples_analyzed"] = (
            attribution_results.get("n_samples", 0)
        )

        self.logger.debug(
            f"Stored {len(self.last_attribution_metrics)} SHAP attribution metrics for dashboard/wandb"
        )
        ...
        try:
            ...
            if (
                ...
                for key, value in self.last_attribution_metrics.items():
                    if "branch_" in key and "_importance" in key:
                        branch = key.replace("shap_branch_", "").replace(
                            "_importance", ""
                        )
            ...
            if len(self.state_buffer) >= 5:
                ...
                if attribution_results and "branch_importance" in attribution_results:
                    for branch, importance in attribution_results[
                        "branch_importance"
                    ].items():
    ...
    def generate_feature_report(
        ...
        try:
            ...
            if len(self.state_buffer) >= 10:
                for state in list(self.state_buffer)[-10:]:
                    ...
                    for key, tensor in state.items():
                        if torch.is_tensor(tensor):
                            formatted_state[key] = tensor.to(
                                self.attribution_analyzer.device
                            )
                        else:
            ...
            if states_for_analysis:
                attribution_results = self.attribution_analyzer.analyze_features(
                    states_for_analysis, max_samples=10
                )
                ...
                if attribution_results and "error" not in attribution_results:
                    ...
                    report = {
                        "method": "SHAP",
                        "analysis_summary": attribution_results.get(
                            "summary_stats", {}
                        ),
                        "branch_importance": attribution_results.get(
                            "branch_importance", {}
                        ),
                        "top_features": attribution_results.get("top_features", []),
                        "dead_features": attribution_results.get("dead_features", {}),
                        "feature_interactions": attribution_results.get(
                            "feature_interactions", {}
                        ),
                        "individual_explanations": attribution_results.get(
                    ...
                    self.logger.info(
                        f"Generated comprehensive SHAP feature report with {len(report)} sections"
                    )
                    ...
                    if save_path:
                        ...
                        try:
                            ...
                            self.logger.info(
                                f"SHAP feature report saved to {save_path}"
                            )
                        except Exception as e:
                            self.logger.warning(
                                f"Failed to save report to {save_path}: {e}"
                            )
    ...
    def get_feature_attribution_summary(self) -> Dict[str, Any]:
        ...
        if hasattr(self, "last_attribution_metrics") and self.last_attribution_metrics:
            ...
            summary = {
                ...
                "branches_tracked": len(
                    [
                        k
                        for k in self.last_attribution_metrics.keys()
                        if "branch_" in k and "_importance" in k
                    ]
                ),
            }
            ...
            for i in range(1, 6):  # Top 5
                ...
                if (
                    importance_key in self.last_attribution_metrics
                    and name_key in self.last_attribution_metrics
                ):
                    top_features.append(
                        {
                            "name": self.last_attribution_metrics[name_key],
                            "current": float(
                                self.last_attribution_metrics[importance_key]
                            ),
                            "average": float(
                                self.last_attribution_metrics[importance_key]
                            ),  # No history yet
            ...
            summary["dead_features_count"] = self.last_attribution_metrics.get(
                "shap_dead_features_total", 0
            )

            self.logger.info(
                f"📊 Generated attribution summary with {len(summary)} keys for dashboard"
            )

...
class RewardMetricsCollector(MetricCollector):
    ...
    def __init__(self):
        ...
        # Component tracking
        self.component_data = defaultdict(
            lambda: {"values": [], "count": 0, "total": 0.0, "last_value": 0.0}
        )
    ...
    def register_component(self, component_name: str, component_type: str):
        ...
        self.register_metric(
            f"component.{component_name}.value",
            MetricMetadata(
                category=MetricCategory.TRAINING,
                metric_type=MetricType.GAUGE,
        ...
        self.register_metric(
            f"component.{component_name}.mean",
            MetricMetadata(
                category=MetricCategory.TRAINING,
                metric_type=MetricType.GAUGE,
        ...
        self.register_metric(
            f"component.{component_name}.trigger_rate",
            MetricMetadata(
                category=MetricCategory.TRAINING,
                metric_type=MetricType.PERCENTAGE,
        ...
        self.register_metric(
            f"component.{component_name}.contribution_pct",
            MetricMetadata(
                category=MetricCategory.TRAINING,
                metric_type=MetricType.PERCENTAGE,
        ...
        self.register_metric(
            f"component.{component_name}.cumulative",
            MetricMetadata(
                category=MetricCategory.TRAINING,
                metric_type=MetricType.GAUGE,
        ...
        self.register_metric(
            f"component.{component_name}.volatility",
            MetricMetadata(
                category=MetricCategory.TRAINING,
                metric_type=MetricType.GAUGE,
    ...
    def collect(self) -> Dict[str, MetricValue]:
        ...
        try:
            # Episode metrics
            metrics[f"{self.category.value}.{self.name}.episode_reward"] = MetricValue(
                self.episode_total_reward
            )

            if self.step_rewards:
                mean_reward = np.mean(self.step_rewards)
                metrics[f"{self.category.value}.{self.name}.mean_step_reward"] = (
                ...
                metrics[f"{self.category.value}.{self.name}.reward_sparsity"] = (
                    MetricValue(sparsity)
                )
                ...
                if positive_rewards:
                    metrics[
                        f"{self.category.value}.{self.name}.positive_reward_magnitude"
                    ] = MetricValue(np.mean(positive_rewards))
                if negative_rewards:
                    metrics[
                        f"{self.category.value}.{self.name}.negative_reward_magnitude"
                    ] = MetricValue(abs(np.mean(negative_rewards)))

            # Component metrics
            for comp_name, comp_data in self.component_data.items():
                # Current value
                metrics[
                    f"{self.category.value}.{self.name}.component.{comp_name}.value"
                ] = MetricValue(comp_data["last_value"])

                # Mean value
                if comp_data["values"]:
                    mean_val = np.mean(comp_data["values"][-100:])  # Last 100 values
                    metrics[
                        f"{self.category.value}.{self.name}.component.{comp_name}.mean"
                    ] = MetricValue(mean_val)

                # Trigger rate
                total_steps = len(self.step_rewards) if self.step_rewards else 1
                trigger_rate = (comp_data["count"] / total_steps) * 100
                metrics[
                    f"{self.category.value}.{self.name}.component.{comp_name}.trigger_rate"
                ] = MetricValue(trigger_rate)

                # Contribution percentage
                if self.episode_total_reward != 0:
                    contribution_pct = (
                        self.episode_component_totals[comp_name]
                        / abs(self.episode_total_reward)
                    ) * 100
                    metrics[
                        f"{self.category.value}.{self.name}.component.{comp_name}.contribution_pct"
                    ] = MetricValue(contribution_pct)

                # Cumulative value
                metrics[
                    f"{self.category.value}.{self.name}.component.{comp_name}.cumulative"
                ] = MetricValue(self.episode_component_totals[comp_name])

                # Volatility
                if len(comp_data["values"]) > 1:
                    volatility = np.std(comp_data["values"])
                    metrics[
                        f"{self.category.value}.{self.name}.component.{comp_name}.volatility"

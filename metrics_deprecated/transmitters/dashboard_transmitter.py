...
class DashboardTransmitter(MetricTransmitter):
    ...
    def __init__(
        self, port: int = 8050, update_interval: float = 0.1, metrics_integrator=None
    ):
        """
        ...
        if not self._is_started:
            try:
                ...
                self._dashboard_thread = threading.Thread(
                    target=lambda: start_dashboard(
                        port=self.port, open_browser=open_browser
                    ),
                    daemon=True,
                )
            ...
            try:
                ...
                with self._buffer_lock:
                    if self._metric_buffer:
                        ...
                        dashboard_update = self._convert_metrics_to_dashboard_format(
                            self._metric_buffer
                        )
                        if dashboard_update:
                            # Debug logging for actions and rewards
                            action_metrics = [
                                k for k in dashboard_update.keys() if "action" in k
                            ]
                            reward_metrics = [
                                k for k in dashboard_update.keys() if "reward" in k
                            ]
                            if action_metrics:
                                self.logger.debug(
                                    f"Transmitting action metrics: {action_metrics}"
                                )
                            if reward_metrics:
                                self.logger.debug(
                                    f"Transmitting reward metrics: {reward_metrics}"
                                )
    ...
    def _convert_metrics_to_dashboard_format(
        ...
        for metric_name, metric_data in metrics.items():
            ...
            if "training.ppo" in metric_name or "model.model" in metric_name:
                ...
                if "episode_count" in metric_name:
                    dashboard_data["total_episodes"] = (
                        value  # Global episode count across all sessions
                    )
                    ...
                    dashboard_data["updates_per_second"] = (
                        value / 3600
                ...
                "execution.environment" in metric_name
                or "environment.environment" in metric_name
            ):
                # Action counts
        ...
        try:
            if event_name in ["training_update", "ppo_metrics"]:
                ...
                curriculum_data = {
                    ...
                    "curriculum_progress": event_data.get(
                        "stage_progress", event_data.get("progress", 0.0)
                    ),
                    "curriculum_min_quality": event_data.get("min_quality", 0.8),
                    "total_episodes_for_curriculum": event_data.get(
                        "total_episodes", 0
                    ),
                ...
                self.logger.debug(
                    f"Received curriculum_progress event: {curriculum_data}"
                )
                ...
                momentum_data = {
                    ...
                    "cycles_completed": day_info.get(
                        "cycles_completed", 0
                    ),  # Map to cycles_completed field too
                    "total_momentum_days_used": day_info.get("total_days_used", 0),
                }
                self.logger.debug(
                    f"Received momentum_day_change event: {momentum_data}"
                )
                ...
                reset_point_data = {
                    ...
                    "selected_reset_point_timestamp": event_data.get(
                        "selected_timestamp", ""
                    ),
                    "total_available_points": event_data.get(
                        "total_available_points", 0
                    ),
                    "points_used_in_cycle": event_data.get("points_used_in_cycle", 0),
                    "points_remaining_in_cycle": event_data.get(
                ...
                cycle_data = {
                    ...
                    "target_cycles_per_day": event_data.get(
                        "target_cycles_per_day", 10
                    ),
                    "cycles_remaining_for_day_switch": event_data.get(
                        "cycles_remaining_for_day_switch", 10
                    ),
                    "day_switch_progress_pct": event_data.get(
                        "day_switch_progress_pct", 0.0
                    ),
                    "episodes_on_current_day": event_data.get(

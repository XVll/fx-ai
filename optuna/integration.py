...
class OptunaConfigManager:
    ...
    @staticmethod
    def merge_configs(base_config: Dict[str, Any], optuna_params: Dict[str, Any]) -> Dict[str, Any]:
...
def optimize_training_parameters(
    ...
    def objective(trial):
        params = {
            'training.learning_rate': trial.suggest_float('training.learning_rate', 1e-5, 1e-3, log=True),
            'training.batch_size': trial.suggest_categorical('training.batch_size', [32, 64, 128]),
            'training.n_epochs': trial.suggest_int('training.n_epochs', 4, 12),
            'training.gamma': trial.suggest_float('training.gamma', 0.95, 0.999),
            'training.gae_lambda': trial.suggest_float('training.gae_lambda', 0.9, 0.98),
            'training.entropy_coef': trial.suggest_float('training.entropy_coef', 1e-4, 1e-1, log=True),
        }

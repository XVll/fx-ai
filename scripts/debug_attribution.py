...
def test_captum_attribution():
    ...
    try:
        ...
        if "model" in str(e).lower():
            print(
                f"✅ CaptumFeatureAnalyzer class loadable (expected model error: {e})"
            )
        else:
...
def check_metrics_system():
    ...
    try:
        ...
        collector = ModelInternalsCollector(
            model=None, feature_names=None, enable_attribution=True
        )

        print("✅ ModelInternalsCollector created")

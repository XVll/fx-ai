...
def test_percentage_reward_system():
    ...
    print(
        f"✅ Holding penalty coefficient: {config.env.reward.holding_penalty_coefficient}"
    )
    print(
        f"✅ Drawdown penalty coefficient: {config.env.reward.drawdown_penalty_coefficient}"
    )
    print(
        f"✅ Action penalty coefficient: {config.env.reward.action_penalty_coefficient}"
    )
    print(
        f"✅ Quick profit bonus coefficient: {config.env.reward.quick_profit_bonus_coefficient}"
    )
    ...
    for i, test_case in enumerate(test_cases):
        ...
        portfolio_before = {"realized_pnl_session": 0.0, "total_equity": account_value}
        portfolio_after = {
        ...
        actual_percentage = (
            (profit_dollars - 7.0) / account_value * 100
        )  # Subtract costs
        ...
        if abs(pnl_reward - expected_reward) <= tolerance:
            print("   ✅ PASS: Reward scales correctly with percentage")
        else:
    ...
    print(
        f"   📊 Default coefficient (100): reward = {expected_default:.1f} (expected)"
    )
    ...
    if abs(custom_pnl_reward - expected_custom) <= 0.1:  # Should be roughly half
        print("   ✅ PASS: Coefficient scaling works correctly")
    else:
        print(
            f"   ❌ FAIL: Expected {expected_custom:.1f}, got {custom_pnl_reward:.1f}"
        )
        all_passed = False

...
def create_test_fill(side: OrderSideEnum, quantity: float, price: float, timestamp: datetime = None) -> FillDetails:
    """Create a test fill."""
...
def validate_basic_trade_flow():
    ...
    logger.info(f"Processing BUY fill: {buy_fill.executed_quantity} @ ${buy_fill.executed_price}")
    
    enriched_buy = portfolio.process_fill(buy_fill)
    logger.info(f"Enriched BUY - closes_position: {enriched_buy.closes_position}, realized_pnl: {enriched_buy.realized_pnl}")
    ...
    logger.info(f"Position after BUY: side={position.side.value}, qty={position.quantity}, avg_price=${position.avg_price:.4f}")
    ...
    logger.info(f"Processing SELL fill: {sell_fill.executed_quantity} @ ${sell_fill.executed_price}")
    
    enriched_sell = portfolio.process_fill(sell_fill)
    logger.info(f"Enriched SELL - closes_position: {enriched_sell.closes_position}, realized_pnl: {enriched_sell.realized_pnl}")
    ...
    logger.info(f"Position after SELL: side={final_position.side.value}, qty={final_position.quantity}")
    ...
    if abs(actual_realized_pnl - expected_gross_pnl) < 0.01:  # Check against gross P&L (commission/fees handled separately)
        logger.info("✅ P&L calculation CORRECT")
    else:
        logger.error(f"❌ P&L calculation INCORRECT - Expected: ${expected_gross_pnl:.2f}, Actual: ${actual_realized_pnl:.2f}")

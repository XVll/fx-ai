...
class SharedDashboardState:
    ...
    clip_fraction_history: Deque[float] = field(
        default_factory=lambda: deque(maxlen=10)
    )
    kl_divergence_history: Deque[float] = field(
        default_factory=lambda: deque(maxlen=10)
    )
    learning_rate_history: Deque[float] = field(
        default_factory=lambda: deque(maxlen=10)
    )
    explained_variance_history: Deque[float] = field(
        default_factory=lambda: deque(maxlen=10)
    )
    ...
    ppo_metrics_history: Deque[Dict[str, Any]] = field(
        default_factory=lambda: deque(maxlen=200)
    )
    reward_history: Deque[Dict[str, Any]] = field(
        default_factory=lambda: deque(maxlen=200)
    )
    candle_data_1m: List[Dict[str, Any]] = field(
    ...
    recent_actions: Deque[Dict[str, Any]] = field(
        default_factory=lambda: deque(maxlen=20)
    )
    action_distribution: Dict[str, int] = field(
        default_factory=lambda: {"HOLD": 0, "BUY": 0, "SELL": 0}
    )
    ...
    episode_action_distribution: Dict[str, int] = field(
        default_factory=lambda: {"HOLD": 0, "BUY": 0, "SELL": 0}
    )
    session_action_distribution: Dict[str, int] = field(
        default_factory=lambda: {"HOLD": 0, "BUY": 0, "SELL": 0}
    )
...
class DashboardStateManager:
            ...
            if event.event_type == EventType.MARKET_UPDATE:
                ...
                if market_timestamp:
                    try:
                        ...
                        if (
                            self._state.position_side != "FLAT"
                            and self._state.position_entry_timestamp
                            ...
                            try:
                                ...
                                if (
                                    abs(
                                        new_hold_time
                                        - self._state.position_hold_time_seconds
                                    )
                                    >= 1
                                ):
                                    self._state.position_hold_time_seconds = (
                                    ...
                                    if new_hold_time % 60 == 0:
                                        ...
                                        logger.debug(
                                            f"Hold time updated: {new_hold_time}s ({new_hold_time // 60}m) for {self._state.position_side} position"
                                        )
                                ...
                                logger = logging.getLogger(__name__)
                                logger.debug(
                                    f"Market update hold time calculation error: {e}, current_ts: {self._state.current_timestamp}, entry_ts: {self._state.position_entry_timestamp}"
                                )
                ...
                if entry_timestamp and self._state.position_side != "FLAT":
                    ...
                    try:
                        ...
                        if entry_ts.tz is not None:
                            ...
                            entry_ts_ny = entry_ts.tz_convert(
                                "America/New_York"
                            ).tz_localize(None)
                        else:
                        ...
                        if self._state.current_timestamp:
                            try:
                                ...
                                logger = logging.getLogger(__name__)
                                logger.debug(
                                    f"Hold time calculation error: {e}, current_ts: {self._state.current_timestamp}, entry_ts: {self._state.position_entry_timestamp}"
                                )
                        ...
                        logger = logging.getLogger(__name__)
                        logger.debug(
                            f"Entry timestamp parsing error: {e}, timestamp: {entry_timestamp}"
                        )
                ...
                self._state.position_pnl_dollar = (
                    self._state.unrealized_pnl
                )  # Use portfolio simulator's unrealized P&L
                ...
                if self._state.position_qty > 0 and self._state.avg_entry_price > 0:
                    ...
                    self._state.position_pnl_percent = (
                        (self._state.position_pnl_dollar / entry_value) * 100
                        if entry_value > 0
                        else 0
                    )
                else:
                ...
                if action in self._state.episode_action_distribution:
                    ...
                    logger = logging.getLogger(__name__)
                    logger.debug(
                        f"Action {action} recorded. Episode counts: {self._state.episode_action_distribution}, Session total: {self._state.action_distribution}"
                    )
    ...
    def update_metrics(self, metrics: Dict[str, Any]):
        ...
        with self._lock:
            ...
            for key in [
                "current_momentum_day_date",
                "current_momentum_day_quality",
                "episodes_on_current_day",
                "reset_point_cycles_completed",
            ...
            # Enhanced reset point tracking
            for key in [
                "selected_reset_point_index",
                "selected_reset_point_timestamp",
                "total_available_points",
                "points_used_in_cycle",
                "points_remaining_in_cycle",
            ]:
                if key in metrics:
                    setattr(self._state, key, metrics[key])
            ...
            for key in [
                ...
                "cycles_remaining_for_day_switch",
            ...
            if "execution.environment.action_hold_count" in metrics:
                self._state.session_action_distribution["HOLD"] = int(
                    metrics["execution.environment.action_hold_count"]
                )
                action_updated = True
            if "execution.environment.action_buy_count" in metrics:
                self._state.session_action_distribution["BUY"] = int(
                    metrics["execution.environment.action_buy_count"]
                )
                action_updated = True
            if "execution.environment.action_sell_count" in metrics:
                self._state.session_action_distribution["SELL"] = int(
                    metrics["execution.environment.action_sell_count"]
                )
                action_updated = True
            ...
            if "episode_action_hold_count" in metrics:
                self._state.episode_action_distribution["HOLD"] = int(
                    metrics["episode_action_hold_count"]
                )
                action_updated = True
            if "episode_action_buy_count" in metrics:
                self._state.episode_action_distribution["BUY"] = int(
                    metrics["episode_action_buy_count"]
                )
                action_updated = True
            if "episode_action_sell_count" in metrics:
                self._state.episode_action_distribution["SELL"] = int(
                    metrics["episode_action_sell_count"]
                )
                action_updated = True
            ...
            if action_updated:
                ...
                logger = logging.getLogger(__name__)
                logger.debug(
                    f"Action counts updated - Episode: {self._state.episode_action_distribution}, Session: {self._state.session_action_distribution}"
                )
    ...
    def reset_episode(self):
        ...
        with self._lock:
            ...
            logger = logging.getLogger(__name__)
            logger.debug(
                f"Resetting episode - Previous episode actions: {self._state.episode_action_distribution}"
            )
            ...
            logger.debug(
                f"Episode reset complete - New episode actions: {self._state.episode_action_distribution}"
            )

...
class DashboardServer:
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Graph(
                                    id="candlestick-chart", style={"height": "500px"}
                                ),
                            ],
                            style=self._card_style(),
                        ),
        ...
        def update_dashboard(n):
            ...
            market_info = html.Div(
                [
                    ...
                    self._info_row(
                        "Session", trading_hours, color=DARK_THEME["accent_orange"]
                    ),
                    self._info_row(
                        "Price",
            ...
            if state.position_side != "FLAT":
                ...
                position_section = [
                    self._info_row(
                        "Side", state.position_side, color=DARK_THEME["accent_blue"]
                    ),
                    ...
                    self._info_row(
                        "P&L %", f"{state.position_pnl_percent:.1f}%", color=pnl_color
                    ),
                    self._info_row(
                        "Hold Time",
            ...
            portfolio_section = [
                ...
                self._info_row(
                    "Session P&L", f"${state.session_pnl:.2f}", color=session_pnl_color
                ),
            ...
            performance_metrics_info = dash_table.DataTable(
                ...
                style_data_conditional=[
                    # Highlight win rates
                    {
                        "if": {
                            "column_id": "Episode",
                            "filter_query": '{Metric} = "Win Rate %"',
                        },
                        "color": DARK_THEME["accent_green"]
                        if episode_win_rate >= 50
                        else DARK_THEME["accent_red"],
                    },
                    {
                        "if": {
                    ...
                    # Highlight drawdown
                    {
                        ...
                        if state.max_drawdown > 10
                        else DARK_THEME["accent_orange"],
                    },
            ...
            episode_actions = getattr(
                state, "episode_action_distribution", {"HOLD": 0, "BUY": 0, "SELL": 0}
            )
            session_actions = getattr(
                state, "session_action_distribution", {"HOLD": 0, "BUY": 0, "SELL": 0}
            )

            # Fallback logic for both episode and session actions
            event_stream_actions = getattr(
                state, "action_distribution", {"HOLD": 0, "BUY": 0, "SELL": 0}
            )

            # If session actions are empty but event stream has data, use event stream as session fallback
            if (
                all(v == 0 for v in session_actions.values())
                and sum(event_stream_actions.values()) > 0
            ):
            ...
            if (
                all(v == 0 for v in episode_actions.values())
                and sum(event_stream_actions.values()) > 0
            ):
            ...
            episode_reward_total = sum(
                getattr(state, "episode_reward_components", {}).values()
            )
            ...
            ppo_metrics_data = [
                ...
                (
                    "KL Divergence",
                    getattr(state, "kl_divergence", 0.0),
                    getattr(state, "kl_divergence_history", deque()),
                ),
                (
                    "Clip Fraction",
                    getattr(state, "clip_fraction", 0.0),
                    getattr(state, "clip_fraction_history", deque()),
                ),
                (
                    "Learning Rate",
                    getattr(state, "learning_rate", 0.0),
                    getattr(state, "learning_rate_history", deque()),
                ),
                (
                    "Explained Var",
                    getattr(state, "explained_variance", 0.0),
                    getattr(state, "explained_variance_history", deque()),
                ),
                (
                    "Mean Reward",
                    getattr(state, "mean_episode_reward", 0.0),
                    getattr(state, "mean_episode_reward_history", deque()),
                ),
            ]
            ...
            ppo_content = html.Div(
                [
                    self._metric_with_sparkline(
                        ...
                        value,
                        history,
                        *self._get_ppo_metric_guidance(
                            label, value, list(history) if history else []
                        ),
                    )
            ...
            day_info_color = (
                DARK_THEME["accent_green"]
                if state.current_momentum_day_quality >= 0.7
                else DARK_THEME["accent_blue"]
                if state.current_momentum_day_quality >= 0.5
            ...
            cycles_completed = getattr(
                state,
                "cycles_completed",
                getattr(state, "reset_point_cycles_completed", 0),
            )
            ...
            logger.debug(
                f"Curriculum values - progress: {progress_pct}%, cycles: {cycles_completed}/{target_cycles}, day_switch: {day_switch_progress}%, roc_range: {roc_range}, activity_range: {activity_range}"
            )
            logger.debug(
                f"Reset point tracking - total_available: {total_available}, points_used: {points_used}, episodes_on_day: {getattr(state, 'episodes_on_current_day', 0)}"
            )
            ...
            env_content = html.Div(
                [
                    ...
                    self._info_row(
                        "Points Used",
                        f"{points_used}/{total_available}"
                        if total_available > 0
                        else "0/0",
                        color=DARK_THEME["accent_green"]
                        if points_used < total_available
                        else DARK_THEME["accent_orange"],
                    ),
                    self._info_row(
                        "Cycle Progress",
                    ...
                    self._info_row(
                        "Day Switch",
                        f"{day_switch_progress:.0f}%",
                        color=DARK_THEME["accent_green"]
                        if day_switch_progress < 90
                        else DARK_THEME["accent_orange"],
                    ),
                    # Day information
                    html.Hr(
                    ...
                    self._info_row(
                        "Day",
                        state.current_momentum_day_date or "N/A",
                        color=day_info_color,
                    ),
                    self._info_row(
                        "Day Score",
                        f"{state.current_momentum_day_quality:.3f}"
                        if state.current_momentum_day_quality > 0
                        else "N/A",
                        color=day_info_color,
                    ),
            ...
            # Attribution panels
            attribution_features_content = self._create_attribution_features_content(
                state
            )
            attribution_quality_content = self._create_attribution_quality_content(
                state
            )
            ...
            return (
                ...
                reward_table,
                env_content,
                attribution_features_content,
                attribution_quality_content,
                candlestick_chart,
                footer,
            )

    def _info_row(
        self, label: str, value: str, color: Optional[str] = None
    ) -> html.Div:
    ...
    def _create_progress_section(
        self, title: str, current: int, total: int, text: str
    ) -> Optional[html.Div]:
        ...
        if "rollout" in stage.lower():
            ...
            if rollout_total > 0:
                ...
                return self._create_progress_bar_with_text(
                    "Current Stage", progress_pct, text
                )
    ...
    def _create_progress_bar_with_text(
        self, title: str, progress_pct: float, text: str
    ) -> html.Div:
                ...
                if len(history_list) >= 1:  # Show chart even with just 1 point
                    ...
                    if len(history_list) == 1:
                        ...
                        chart_data = history_list[
                            -20:
                        ]  # Last 20 values for better trend visibility
    ...
    def _get_ppo_metric_guidance(
        self, label: str, value: float, history: List[float]
    ) -> tuple[str, str]:
    ...
    def _create_price_chart(self, state) -> go.Figure:
        ...
        if not candle_data:
            ...
            if candles:
                ...
                executions_data = (
                    list(state.recent_executions) if state.recent_executions else []
                )
                if executions_data:
    ...
    def _create_attribution_features_content(self, state) -> html.Div:
        ...
        try:
            ...
            if not attribution_summary or not attribution_summary.get(
            ...
            for branch, features in top_features.items():
                if features:  # List of {'name': str, 'importance': float}
                    # Branch header
                    branch_color = branch_colors.get(
                        branch, DARK_THEME["accent_purple"]
                    )
                    ...
                    for feature in features[:3]:
                        ...
                        importance_pct = min(
                            100, abs(importance) * 1000
                        )  # Scale for visibility
                        bar_color = (
                            branch_color if importance > 0 else DARK_THEME["text_muted"]
                        )
            ...
            return html.Div(
                feature_elements, style={"maxHeight": "120px", "overflowY": "auto"}
            )
            ...
            return html.Div(
                [
                    html.Div(
                        f"Error: {str(e)}",
                        style={"color": DARK_THEME["accent_red"], "fontSize": "10px"},
                    )
                ]
            )
        ...
        try:
            ...
            if not attribution_summary:
                return html.Div(
                    [
                        html.Div(
                            "📊 Waiting for attribution analysis...",
                            style={
                                "color": DARK_THEME["text_secondary"],
                                "fontSize": "11px",
                                "textAlign": "center",
            ...
            if max_attrs:
                ...
                for branch, max_attr in max_attrs.items():
                    ...
                    quality_elements.append(
                        self._info_row(
                            f"{branch.upper()}", f"{max_attr:.3f}", branch_color
                        )
                    )

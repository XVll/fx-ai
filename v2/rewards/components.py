...
from v2.rewards.core import RewardComponent, RewardMetadata, RewardState, RewardType
from v2.simulators.portfolio_simulator import PositionSideEnum
...
class PnLReward(RewardComponent):
    ...
    def calculate(self, state: RewardState) -> Tuple[float, Dict[str, Any]]:
        ...
        realized_pnl_after = state.portfolio_after_fills.get(
        ...
        for fill in state.fill_details:
            ...
            slippage = (
                fill.slippage_cost_total
                if fill.slippage_cost_total is not None
                else 0.0
            )
        ...
        pnl_percentage = (
            (net_pnl_dollars / account_value * 100.0) if account_value > 0 else 0.0
        )
...
class DrawdownPenalty(RewardComponent):
    ...
    def calculate(self, state: RewardState) -> Tuple[float, Dict[str, Any]]:
        ...
        diagnostics = {
            ...
            "account_value": account_value,
            "drawdown_percentage": (abs(unrealized_pnl) / account_value * 100)
            if account_value > 0 and unrealized_pnl < 0
            else 0,
        }
...
class ProfitGivebackPenalty(RewardComponent):
    ...
    def calculate(self, state: RewardState) -> Tuple[float, Dict[str, Any]]:
        ...
        if max_unrealized_pnl is not None and max_unrealized_pnl > 0:
            ...
            if profit_giveback > 0:
                ...
                if giveback_ratio > giveback_threshold:
                    ...
                    account_value = state.portfolio_next.get("total_equity", 25000.0)
                    profit_percentage = (
                        profit_giveback / account_value if account_value > 0 else 0.0
                    )

                    coefficient = self.config.get(
                        "profit_giveback_penalty_coefficient", 10.0
                    )
        ...
        diagnostics = {
            ...
            "profit_giveback": max_unrealized_pnl - current_unrealized_pnl
            if max_unrealized_pnl
            else 0.0,
            "giveback_ratio": (max_unrealized_pnl - current_unrealized_pnl)
            / max_unrealized_pnl
            if max_unrealized_pnl and max_unrealized_pnl > 0
            else 0.0,
        }
...
class MaxDrawdownPenalty(RewardComponent):
    ...
    def calculate(self, state: RewardState) -> Tuple[float, Dict[str, Any]]:
        ...
        if min_unrealized_pnl is not None and min_unrealized_pnl < 0:
            ...
            account_value = state.portfolio_next.get("total_equity", 25000.0)
            loss_percentage = (
                abs(min_unrealized_pnl) / account_value if account_value > 0 else 0.0
            )

            # Apply escalating penalty for larger MAE
            mae_threshold = self.config.get(
        ...
        diagnostics = {
            ...
            "loss_percentage": abs(min_unrealized_pnl)
            / state.portfolio_next.get("total_equity", 25000.0)
            * 100
            if min_unrealized_pnl and min_unrealized_pnl < 0
            else 0.0,
            "mae_exceeded": min_unrealized_pnl is not None
            and abs(min_unrealized_pnl)
            / state.portfolio_next.get("total_equity", 25000.0)
            > self.config.get("max_drawdown_threshold_percent", 0.01)
            if min_unrealized_pnl
            else False,
        }
        ...
        for fill in state.fill_details:
            if fill.closes_position:
                ...
                if realized_pnl > 0:  # Profitable trade
                    ...
                    account_value = state.portfolio_next.get("total_equity", 25000.0)
                    profit_percentage = (
                        realized_pnl / account_value if account_value > 0 else 0.0
                    )

                    # Scale bonus with profit size (quadratic for larger profits)
                    coefficient = self.config.get(
                        "profit_closing_bonus_coefficient", 5.0
                    )
                    bonus += coefficient * (
...
class CleanTradeBonus(RewardComponent):
        ...
        for fill in state.fill_details:
            if fill.closes_position:
                ...
                if realized_pnl > 0:  # Only reward profitable trades
                    ...
                    position_value = (
                        abs(realized_pnl) / 0.02
                        if realized_pnl != 0
                        else account_value * 0.1
                    )

                    # Calculate bonus for this trade
                    trade_bonus, trade_details = self._calculate_clean_trade_bonus(
                        mae, mfe, realized_pnl, position_value
                    )
                    bonus += trade_bonus
    ...
    def _calculate_clean_trade_bonus(
        self, mae: float, mfe: float, final_pnl: float, position_value: float
        ...
        # Configuration
        max_mae_threshold = self.config.get(
            "max_mae_threshold", 0.02
        )  # 2% max drawdown
        min_gain_threshold = self.config.get(
        ...
        if final_pct < min_gain_threshold:
            return 0.0, {
                ...
                "final_pct": final_pct,
                "min_threshold": min_gain_threshold,
            }
...
class InactivityPenalty(RewardComponent):
    ...
    def calculate(self, state: RewardState) -> Tuple[float, Dict[str, Any]]:
        ...
        diagnostics = {"is_hold_action": is_hold_action, "hold_penalty": penalty}
    ...
    def calculate(self, state: RewardState) -> Tuple[float, Dict[str, Any]]:
        ...
        if state.terminated and state.termination_reason:
            ...
            if "BANKRUPTCY" in reason:
                bankruptcy_coefficient = self.config.get(
        ...
        diagnostics = {
            ...
            "is_bankruptcy": "BANKRUPTCY" in str(state.termination_reason)
            if state.termination_reason
            else False,
        }

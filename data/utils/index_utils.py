...
class IndexManager:
    ...
    def __init__(
        self,
        momentum_days_df: Optional[pd.DataFrame] = None,
        ...
        """
        self.momentum_days = (
            momentum_days_df if momentum_days_df is not None else pd.DataFrame()
        )
        self.reset_points = (
            reset_points_df if reset_points_df is not None else pd.DataFrame()
        )
    ...
    def get_day_reset_points(
        self, symbol: str, date: datetime, max_points: Optional[int] = None
    ) -> pd.DataFrame:
    ...
    def _apply_curriculum_filter(
        self, days_df: pd.DataFrame, stage: str
    ) -> pd.DataFrame:
        ...
        if stage == "early":
            ...
            category_weights = {"good": 0.2, "very_good": 0.3, "excellent": 0.5}
    ...
    def _apply_diversity_weights(self, days_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "curriculum_weight" in days_df.columns:
            days_df["selection_weight"] = (
                days_df["curriculum_weight"] * days_df["usage_weight"]
            )
        else:
    ...
    def get_quality_distribution(
        self, symbol: Optional[str] = None, bins: int = 10
    ) -> pd.DataFrame:
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        most_used = [
            {"symbol": k[0], "date": k[1], "count": v} for k, v in sorted_usage[:10]
        ]
    ...
    def get_momentum_days(
        self, symbol: str, min_quality: float = 0.0
    ) -> List[MomentumDay]:
    ...
    def get_next_day(
        self, current_date: Optional[datetime], min_quality: float = 0.0
    ) -> Optional[MomentumDay]:

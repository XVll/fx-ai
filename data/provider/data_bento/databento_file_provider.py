...
class DatabentoFileProvider(HistoricalDataProvider):
        ...
        if False:  # symbol_info_file and os.path.exists(symbol_info_file):
            try:
                ...
                if "symbol" in df_info.columns:
                    ...
                    self.logger.info(
                        f"Loaded symbol info for {len(self._symbol_info_map)} symbols"
                    )
                else:
                    self.logger.warning(
                        f"'symbol' column not found in {symbol_info_file}"
                    )
    ...
    def _scan_and_build_metadata_index(self):
        ...
        for root, dirs, files in os.walk(self.data_dir, topdown=True):
            if "metadata.json" in files and "manifest.json" in files:
                ...
                try:
                    ...
                    if not all(
                        [schema, symbols, start_ns is not None, end_ns is not None]
                    ):
                        ...
                        if filename.endswith((".dbn", ".dbn.zst")):
                            ...
                            if schema.lower() in filename.lower():
                                ...
                                if os.path.exists(file_path):
                                    ...
                                    self._log(
                                        f"DBN file '{file_path}' listed in manifest but not found on disk.",
                                        logging.WARNING,
                                    )
        ...
        self.logger.info(
            f"Found {found_dirs} directories, indexed {len(self.metadata_index)} files"
        )

    def _find_dbn_files(
        self, schema: str, symbol: str, start_time: pd.Timestamp, end_time: pd.Timestamp
    ) -> List[str]:
        ...
        for file_path in files:
            ...
            if not df.empty:
                ...
                if df.index.tz is None:
                    self.logger.warning(
                        "Input DataFrame index is timezone-naive. Localizing to UTC."
                    )
                    df.index = df.index.tz_localize("UTC")
                elif (
                    str(df.index.tz).upper() != "UTC"
                ):  # Check if it's aware but not UTC
                    self.logger.info(
                        f"Input DataFrame index is timezone-aware ({df.index.tz}). Converting to UTC."
                    )
    ...
    def get_trades(
        ...
        if not files:
            self._log(
                f"No trade files found for {symbol} from {start_utc} to {end_utc}"
            )
            return pd.DataFrame()
    ...
    def get_quotes(
        ...
        if not files:
            self._log(
                f"No quote files found for {symbol} from {start_utc} to {end_utc}"
            )
            return pd.DataFrame()
    ...
    def get_bars(
        ...
        if timeframe not in ["1s", "1m", "5m", "1d"]:
            raise ValueError(
                f"Unsupported timeframe: {timeframe}. Supported: 1s, 1m, 5m, 1d"
            )
    ...
    def _build_5m_bars_from_1m(
        ...
        if not all(col in bars_1m.columns for col in required_cols):
            self._log(
                f"Missing required columns in 1m bars: {set(required_cols) - set(bars_1m.columns)}"
            )
            return pd.DataFrame()
    ...
    def get_status(
        ...
        if not files:
            self._log(
                f"No status files found for {symbol} from {start_utc} to {end_utc}"
            )
            return pd.DataFrame()
        ...
        if "action" in raw_df.columns:
            ...
            std_df["status"] = raw_df["action"].map(
                lambda x: status_map.get(x, f"UNKNOWN_{x}")
            )
        else:
        ...
        if "reason" in raw_df.columns:
            ...
            std_df["reason"] = raw_df["reason"].map(
                lambda x: reason_map.get(x, f"UNKNOWN_{x}")
            )
        else:
        ...
        if "is_short_sell_restricted" in raw_df.columns:
            std_df["is_short_sell_restricted"] = (
                raw_df["is_short_sell_restricted"] == "Y"
            )
        else:
            # Check for SHORT_SELL_RESTRICTION status
            if "status" in std_df.columns:
                std_df["is_short_sell_restricted"] = (
                    std_df["status"] == "SHORT_SELL_RESTRICTION"
                )
            else:

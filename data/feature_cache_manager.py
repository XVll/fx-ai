...
class FeatureCacheManager:
    ...
    def __init__(self, cache_dir: str = "cache/features", logger: Optional[logging.Logger] = None):
        ...
        self.logger.info(f"FeatureCacheManager initialized with cache_dir: {self.cache_dir}")
    ...
    def _load_metadata(self, symbol: str, cache_date: date) -> Optional[CacheMetadata]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to load metadata for {symbol} {cache_date}: {e}")
            return None
    ...
    def _save_metadata(self, metadata: CacheMetadata) -> bool:
        ...
        try:
            ...
            data = {
                ...
                'cached_timestamps': [ts.isoformat() for ts in metadata.cached_timestamps],
            ...
            self.logger.error(f"Failed to save metadata for {metadata.symbol} {metadata.date}: {e}")
            return False
    
    def _load_cache_data(self, symbol: str, cache_date: date) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to load cache data for {symbol} {cache_date}: {e}")
            return None
    
    def _save_cache_data(self, data: Dict[str, Any], symbol: str, cache_date: date) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to save cache data for {symbol} {cache_date}: {e}")
            return False
    ...
    def _invalidate_cache(self, symbol: str, cache_date: date) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to invalidate cache for {symbol} {cache_date}: {e}")
    
    def load_session(self, symbol: str, cache_date: date, feature_manager: SimpleFeatureManager) -> bool:
        """Load cache session for a symbol/date, with automatic invalidation check"""
        try:
            # Check if already loaded
            if self._current_session == (symbol, cache_date):
                self.logger.debug(f"Cache session already loaded for {symbol} {cache_date}")
                return True
            ...
            if metadata is not None and metadata.feature_hash != current_hash:
                self.logger.info(f"Feature structure changed for {symbol} {cache_date}, invalidating cache")
            ...
            if metadata is None:
                # Create new cache
                self.logger.info(f"Creating new feature cache for {symbol} {cache_date}")
                ...
                # Load existing cache
                self.logger.info(f"Loading existing feature cache for {symbol} {cache_date} ({len(metadata.cached_timestamps)} timestamps)")
            ...
            self.logger.error(f"Failed to load cache session for {symbol} {cache_date}: {e}")
            return False
    ...
    def unload_session(self) -> bool:
        ...
        try:
            ...
            if self._dirty_timestamps and self._current_metadata is not None:
                self.logger.info(f"Saving {len(self._dirty_timestamps)} new cached timestamps for {symbol} {cache_date}")
                ...
                success = (
                    self._save_cache_data(self._current_cache, symbol, cache_date) and
                ...
                if success:
                    ...
                    self.logger.error(f"Failed to save cache session for {symbol} {cache_date}")
    ...
    def get_cached_features(self, timestamp: pd.Timestamp) -> Optional[Dict[str, np.ndarray]]:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_size_mb': len(str(self._current_cache)) / (1024 * 1024) if self._current_cache else 0
        }
                ...
                for file_path in symbol_dir.iterdir():
                    if file_path.suffix in ['.pkl', '.json']:
                        try:
                            ...
                            self.logger.warning(f"Failed to check/remove file {file_path}: {e}")
    ...
    def _save_current_progress(self) -> bool:
        ...
        try:
            ...
            if self._current_metadata is not None:
                ...
                if success:
                    self.logger.info(f"Auto-saved {len(self._dirty_timestamps)} cached features for {symbol} {cache_date}")
                    # Clear dirty timestamps since they're now saved
                    self._dirty_timestamps = set()
                    return True
                else:
                    self.logger.error(f"Failed to auto-save cache for {symbol} {cache_date}")
                    return False
            
            return False
            
        except Exception as e:
            self.logger.error(f"Exception during auto-save for {symbol} {cache_date}: {e}")
            return False
...
class TestCallbackDisabling(unittest.TestCase):
    ...
    def test_dashboard_callback_disabled(self):
        ...
        callback = DashboardCallback(
            config={}, dashboard_state=mock_state, enabled=False
        )
    ...
    def test_create_callback_manager_with_enabled_systems(self):
        ...
        with (
            patch("wandb.init"),
            patch("dashboard.shared_state.dashboard_state", Mock()),
        ):
            manager = create_callback_manager(config)
        ...
        self.assertTrue(
            any(isinstance(cb, DashboardCallback) for cb in manager.callbacks)
        )
